...
let rec local_initializer env path init k =
      ...
      let ty_elt, sz =
        ...
        | _ ->
            Diagnostics.fatal_error Diagnostics.no_loc
              "Wrong type for array initializer"
      in
  ...
  | Init_struct (id, fil) ->
      let field_init (fld, i) k =
        local_initializer env
          {edesc= EUnop (Odot fld.fld_name, path); etyp= fld.fld_typ} i k
      in
...
let add_inits_stmt loc inits s =
  List.fold_right (fun e s -> sseq loc {sdesc= Sdo e; sloc= loc} s) inits s
...
let process_compound_literal islocal env ty init =
  ...
  if islocal then (
    ...
    local_variables := (Storage_default, id, ty', None) :: !local_variables ;
...
let rec expand_expr islocal env e =
  let inits = ref [] in
  (* accumulator for initializing assignments *)
  let rec expand e =
    match e.edesc with
    | EConst _ | ESizeof _ | EAlignof _ | EVar _ -> e
    | EUnop (op, e1) -> {edesc= EUnop (op, expand e1); etyp= e.etyp}
    | EBinop (op, e1, e2, ty) ->
        let e1' = expand e1 in
        let e2' =
          match op with
    ...
    | EConditional (e1, e2, e3) ->
        ...
        ; etyp= e.etyp }
    | ECast (ty, e1) -> {edesc= ECast (ty, expand e1); etyp= e.etyp}
    | ECompound (ty, ie) ->
...
and expand_init islocal env i =
  let rec expand i =
    ...
    | Init_single {edesc= ECompound (_, ((Init_struct _ | Init_union _) as i))} ->
        expand i
    | Init_single e -> Init_single (expand_expr islocal env e)
    | Init_array il -> Init_array (List.rev (List.rev_map expand il))
    | Init_struct (id, flds) ->
        Init_struct (id, List.map (fun (f, i) -> (f, expand i)) flds)
    | Init_union (id, fld, i) -> Init_union (id, fld, expand i)
  in
  expand i
...
let debug_annot kind args =
  ...
  ; sdesc=
      Sdo
        ...
        ; edesc=
            ECall
              ( {edesc= EVar debug_id; etyp= debug_ty}
              , intconst kind IInt :: args ) } }

let string_const str =
  let c = CStr str in
  {edesc= EConst c; etyp= type_of_constant c}

let integer_const n = intconst (Int64.of_int n) IInt
...
let debug_lineno (filename, lineno) =
  debug_annot 1L [string_const (Printf.sprintf "#line:%s:%d" filename lineno)]
...
let add_lineno ?(label= false) ctx prev_loc this_loc s =
  if !Clflags.option_g then
    sseq no_loc (debug_scope ctx)
      ( if this_loc <> prev_loc && this_loc <> no_loc && not label then
          sseq no_loc (debug_lineno this_loc) s
      else s )
  else s
...
let block_contains_decl sl =
  List.exists (function {sdesc= Sdecl _} -> true | _ -> false) sl

let next_scope_id = ref 0

let new_scope_id () = incr next_scope_id ; !next_scope_id
...
let rec unblock_stmt env ctx ploc s =
  ...
  | Sskip -> s
  | Sdo e ->
      add_lineno ctx ploc s.sloc {s with sdesc= Sdo (expand_expr true env e)}
  | Sseq (s1, s2) ->
      { s with
        sdesc=
          Sseq (unblock_stmt env ctx ploc s1, unblock_stmt env ctx s1.sloc s2)
      }
  | Sif (e, s1, s2) ->
  ...
  | Swhile (e, s1) ->
      add_lineno ctx ploc s.sloc
        { s with
          sdesc= Swhile (expand_expr true env e, unblock_stmt env ctx s.sloc s1)
        }
  | Sdowhile (s1, e) ->
      add_lineno ctx ploc s.sloc
        { s with
          sdesc=
            Sdowhile (unblock_stmt env ctx s.sloc s1, expand_expr true env e)
        }
  | Sfor (s1, e, s2, s3) ->
      add_lineno ctx ploc s.sloc
        { s with
  ...
  | Sswitch (e, s1) ->
      add_lineno ctx ploc s.sloc
        { s with
          sdesc=
            Sswitch (expand_expr true env e, unblock_stmt env ctx s.sloc s1) }
  | Slabeled (lbl, s1) ->
      let loc, label =
        if s.sloc <> s1.sloc then (s.sloc, false)
          (* Label and code are on different lines *)
        else (ploc, true)
      in
      add_lineno ~label ctx ploc s.sloc
        {s with sdesc= Slabeled (lbl, unblock_stmt env ctx loc s1)}
  | Sgoto lbl -> add_lineno ctx ploc s.sloc s
  | Sreturn None -> add_lineno ctx ploc s.sloc s
  | Sreturn (Some e) ->
      add_lineno ctx ploc s.sloc
        {s with sdesc= Sreturn (Some (expand_expr true env e))}
  | Sblock sl ->
      let ctx' =
        if block_contains_decl sl then
  ...
  | Sasm (attr, template, outputs, inputs, clob) ->
      let expand_asm_operand (lbl, cstr, e) =
        (lbl, cstr, expand_expr true env e)
      in
      add_lineno ctx ploc s.sloc
        { s with
          sdesc=
            Sasm
              ...
              , List.map expand_asm_operand inputs
              , clob ) }
...
and unblock_block env ctx ploc = function
  ...
  | {sdesc= Sdecl d; sloc= loc} :: sl ->
      add_lineno ctx ploc loc
        (process_decl loc env ctx d (unblock_block env ctx loc sl))
  | s :: sl ->
      sseq s.sloc
        (unblock_stmt env ctx ploc s)
  ...
  | g :: gl ->
    ...
    | Genumdef (id, attr, members) ->
        unblock_glob
          ...
          (g :: accu) gl
    | Gpragma _ -> unblock_glob env (g :: accu) gl
