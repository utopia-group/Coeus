...
let color_diagnostics =
  ...
  let activate =
    try Unix.isatty Unix.stderr && term <> "dumb" && term <> "" with _ -> false
  in
  ref activate
...
type msg_class = WarningMsg | ErrorMsg | FatalErrorMsg | SuppressedMsg
...
let key_of_warning w =
  ...
  | Unnamed -> None
  | _ ->
      if !diagnostics_show_option then Some ("-W" ^ string_of_warning w)
      else None

(* Add -Werror to the printed keys *)
let key_add_werror w =
  if !diagnostics_show_option then
    match w with None -> Some "-Werror" | Some s -> Some ("-Werror," ^ s)
  else None
...
let classify_warning w =
  ...
  if List.mem w !active_warnings then
    if List.mem w !error_warnings then
      ...
      if !error_fatal then (FatalErrorMsg, key) else (ErrorMsg, key)
    else (WarningMsg, key)
  else (SuppressedMsg, None)

(* Print color codes if color_diagnostics are enabled *)
let cprintf fmt c =
  if !color_diagnostics then fprintf fmt c else ifprintf fmt c

(* Reset color codes *)
let rsc fmt = cprintf fmt "\027[0m"

(* BOLD *)
let bc fmt = cprintf fmt "\027[1m"

(* RED *)
let rc fmt = cprintf fmt "\027[31;1m"

(* MAGENTA *)
let mc fmt = cprintf fmt "\027[35;1m"

(* Print key (if available) and flush the formatter *)
let pp_key key fmt =
  let key = match key with None -> "" | Some s -> " [" ^ s ^ "]" in
  fprintf fmt "%s%t@." key rsc

(* Different loc output formats *)
type loc_format = Default | MSVC | Vi
...
let parse_loc_format s =
  ...
  let loc_fmt =
    ...
    | "vi" -> Vi
    | s ->
        Printf.eprintf "Invalid value '%s' in '-fdiagnostics-format=%s'\n" s s ;
        exit 2
  in
...
let pp_loc fmt (filename, lineno) =
  ...
  else fprintf fmt "%tccomp:%t " bc rsc

let error key loc fmt =
  incr num_errors ;
  kfprintf (pp_key key) err_formatter ("%a%terror:%t %t" ^^ fmt) pp_loc loc rc
    rsc bc
...
let warning loc ty fmt =
  ...
  match kind with
  | ErrorMsg -> error key loc fmt
  | FatalErrorMsg -> fatal_error key loc fmt
  | WarningMsg ->
      incr num_warnings ;
      kfprintf (pp_key key) err_formatter
        ("%a%twarning:%t %t" ^^ fmt)
        pp_loc loc mc rsc bc
  | SuppressedMsg -> ifprintf err_formatter fmt

let error loc fmt =
  if !error_fatal || error_limit_reached () then fatal_error None loc fmt
  else error None loc fmt

let fatal_error loc fmt = fatal_error None loc fmt

let error_summary () =
  if !num_errors > 0 then (
    eprintf "@[<hov 0>%d error%s detected.@]@." !num_errors
...
let warning_options =
  error_option Unnamed @ error_option Unknown_attribute
  @ error_option Zero_length_array @ error_option Celeven_extension
  @ error_option Gnu_empty_struct @ error_option Missing_declarations
  @ error_option Constant_conversion @ error_option Int_conversion
  @ error_option Varargs @ error_option Implicit_function_declaration
  @ error_option Pointer_type_mismatch
  @ error_option Compare_distinct_pointer_types @ error_option Implicit_int
  @ error_option Main_return_type @ error_option Invalid_noreturn
  @ error_option Return_type @ error_option Literal_range
  @ error_option Unknown_pragmas @ error_option CompCert_conformance
  @ error_option Inline_asm_sdump @ error_option Unused_variable
  @ error_option Unused_parameter @ error_option Wrong_ais_parameter
  @ error_option Unused_ais_parameter
  @ [ (Exact "-Wfatal-errors", Set error_fatal)
    ; (Exact "-fdiagnostics-color", Ignore)
    ; (* Either output supports it or no color *)
    ...
    ; ( _Regexp "-fdiagnostics-format=\\(ccomp\\|msvc\\|vi\\)"
      , Self parse_loc_format ) ]
...
let crash exn =
  let backtrace = Printexc.get_backtrace () and exc = Printexc.to_string exn in
