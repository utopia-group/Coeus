  ...
  | B754_finite (b, m, e) ->
    (match f2 with
     | B754_finite (b0, m0, e1) ->
       if b
       then if b0
            then let s = Pos.eq_dec m m0 in if s then Z.eq_dec e e1 else false
            else false
       else if b0
            then false
            else let s = Pos.eq_dec m m0 in if s then Z.eq_dec e e1 else false
     | _ -> false)
...
let coq_Bconv _ _ prec2 emax2 conv_nan md = function
