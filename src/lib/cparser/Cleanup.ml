...
let addref id =
  if not (IdentSet.mem id !referenced) then (
    (* Printf.printf "Referenced: %s$%d\n" id.name id.stamp; *)
    referenced := IdentSet.add id !referenced ;
    ref_changed := true )

let needed id = IdentSet.mem id !referenced
...
and add_vars vl = List.iter (fun (id, ty) -> add_typ ty) vl

let rec add_exp e =
  add_typ e.etyp ;
  (* perhaps not necessary but play it safe *)
  match e.edesc with
...
and add_init = function
  ...
  | Init_struct (id, il) ->
      addref id ;
      List.iter (fun (_, i) -> add_init i) il
...
let rec add_stmt s =
  ...
  | Sfor (e1, e2, e3, s1) ->
      add_stmt e1 ; add_exp e2 ; add_stmt e3 ; add_stmt s1
  | Sbreak -> ()
  | Scontinue -> ()
  | Sswitch (e, s1) -> add_exp e ; add_stmt s1
  | Slabeled (lbl, s) ->
      (match lbl with Scase e -> add_exp e | _ -> ()) ;
      add_stmt s
  | Sgoto lbl -> ()
  | Sreturn None -> ()
...
let visible_decl (sto, id, ty, init) =
  sto = Storage_default && match ty with TFun _ -> false | _ -> true
...
let rec add_needed_globdecls accu = function
  ...
  | g :: rem ->
    ...
    | Gdecl ((sto, id, ty, init) as decl) ->
        if needed id then (
          add_decl decl ;
          add_needed_globdecls accu rem )
        else add_needed_globdecls (g :: accu) rem
    | Gfundef f ->
        if needed f.fd_name then (
          add_fundef f ;
          add_needed_globdecls accu rem )
        else add_needed_globdecls (g :: accu) rem
    | Gcompositedef (_, id, _, flds) ->
        if needed id then (
          List.iter add_field flds ;
          add_needed_globdecls accu rem )
        else add_needed_globdecls (g :: accu) rem
    | Gtypedef (id, ty) ->
        if needed id then (
    ...
    | Genumdef (id, _, enu) ->
        if needed id || List.exists (fun (id, _, _) -> needed id) enu then (
          add_enum enu ;
          add_needed_globdecls accu rem )
        else add_needed_globdecls (g :: accu) rem
    | _ -> assert false
...
let remove_unused_debug = function _ -> ()
...
let rec simpl_globdecls accu = function
  ...
  | g :: rem ->
      let need =
        ...
        | Gpragma s -> true
      in
      if need then simpl_globdecls (g :: accu) rem
      else (
        remove_unused_debug g.gdesc ;
        simpl_globdecls accu rem )
