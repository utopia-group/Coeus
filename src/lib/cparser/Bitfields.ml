...
type bitfield_info =
  { bf_carrier: string
  ; (* name of underlying regular field *)
    bf_carrier_typ: typ
  ; (* type of underlying regular field *)
    bf_pos: int
  ; (* start bit *)
    bf_size: int
  ; (* size in bit *)
    bf_signed: bool
  ; (* is field signed or unsigned? *)
    bf_signed_res: bool
  ; (* is result of extracting field signed or unsigned? *)
    bf_bool: bool
  (* does field have type _Bool? *) }
...
let bitfield_table : (ident * string, bitfield_info) Hashtbl.t =
  Hashtbl.create 57
...
let unsigned_ikind_for_carrier nbits =
  if nbits <= 8 then IUChar
  else if nbits <= 8 * !config.sizeof_short then IUShort
  else if nbits <= 8 * !config.sizeof_int then IUInt
  else if nbits <= 8 * !config.sizeof_long then IULong
  else if nbits <= 8 * !config.sizeof_longlong then IULongLong
  else assert false
...
let is_signed_enum_bitfield env sid fld eid n =
  ...
  if
    List.for_all
      (fun (_, v, _) -> int_representable v n false)
      info.Env.ei_members
  then false
  else if
    List.for_all
      (fun (_, v, _) -> int_representable v n true)
      info.Env.ei_members
  then true
  else (
    Diagnostics.warning Diagnostics.no_loc Diagnostics.Unnamed
      "not all values of type 'enum %s' can be represented in bit-field '%s' \
       of struct '%s' (%d bits are not enough)"
...
let pack_bitfields env sid ml =
  let rec pack accu pos = function
    ...
    | m :: ms as ml ->
      ...
      | None -> (pos, accu, ml)
      | Some n ->
          if n = 0 then (pos, accu, ms) (* bit width 0 means end of pack *)
          else if pos + n > 8 * !config.sizeof_int then (pos, accu, ml)
            (* doesn't fit in current word *)
          else
            let signed =
              match unroll env m.fld_typ with
              | TInt (ik, _) -> is_signed_ikind ik
              | TEnum (eid, _) ->
                  is_signed_enum_bitfield env sid m.fld_name eid n
              | _ -> assert false
              (* should never happen, checked in Elab *)
            in
            let signed2 =
...
let rec transf_struct_members env id count = function
  | [] -> []
  | m :: ms as ml ->
      if m.fld_bitfield = None then m :: transf_struct_members env id count ms
      else
        ...
        if nbits = 0 then
          ...
          List.iter
            (fun (name, pos, sz, signed, signed2, is_bool) ->
              if name <> "" then
                let pos' =
                  if !config.bitfields_msb_first then
                    (sizeof_ikind carrier_ikind * 8) - pos - sz
                  else pos
                in
                Hashtbl.add bitfield_table (id, name)
                  { bf_carrier= carrier
                  ; bf_carrier_typ= carrier_typ
                  ; bf_pos= pos'
                  ; bf_size= sz
                  ; bf_signed= signed
                  ; bf_signed_res= signed2
                  ; bf_bool= is_bool } )
            bitfields ;
          { fld_name= carrier
          ; fld_typ= carrier_typ
  ...
  | m :: ms ->
    ...
    | Some nbits ->
        ...
        let signed =
          ...
          | TInt (ik, _) -> is_signed_ikind ik
          | TEnum (eid, _) ->
              is_signed_enum_bitfield env id m.fld_name eid nbits
          | _ -> assert false
          (* should never happen, checked in Elab *)
        in
        let signed2 =
        ...
        let pos' =
          if !config.bitfields_msb_first then
            (sizeof_ikind carrier_ikind * 8) - nbits
          else 0
        in
        let is_bool =
          match unroll env m.fld_typ with
          ...
          | _ -> false
        in
        Hashtbl.add bitfield_table (id, m.fld_name)
          { bf_carrier= carrier
          ; bf_carrier_typ= carrier_typ
          ; bf_pos= pos'
          ; bf_size= nbits
...
let right_shift_count bf =
  intconst (Int64.of_int ((8 * !config.sizeof_int) - bf.bf_size)) IInt
...
let insertion_mask bf =
  let m =
    Int64.shift_left (Int64.pred (Int64.shift_left 1L bf.bf_size)) bf.bf_pos
  in
...
let bitfield_extract env bf carrier =
  ...
  if bf.bf_signed_res = bf.bf_signed then e3
...
let bitfield_assign env bf carrier newval =
  ...
  let newval_casted =
    ecast (TInt (IUInt, []))
      (if bf.bf_bool then ecast (TInt (IBool, [])) newval else newval)
  in
  let newval_shifted =
    eshift env Oshl newval_casted (intconst (Int64.of_int bf.bf_pos) IUInt)
  in
...
let bitfield_initializer bf i =
  ...
  | Init_single e ->
      ...
      let e_cast = if bf.bf_bool then ecast (TInt (IBool, [])) e else e in
      ...
      { edesc=
          EBinop
            ( Oshl
            , e_and
            , intconst (Int64.of_int bf.bf_pos) IInt
            , TInt (IUInt, []) )
      ; etyp= TInt (IUInt, []) }
  | _ -> assert false
...
let rec pack_bitfield_init id carrier fld_init_list =
  ...
  | (fld, i) :: rem ->
    ...
    | None -> ([], fld_init_list)
    | Some bf ->
        if bf.bf_carrier <> carrier then ([], fld_init_list)
        else
...
let rec transf_struct_init id fld_init_list =
  ...
  | (fld, i) :: rem ->
    match is_bitfield id fld.fld_name with
    | None -> (fld, i) :: transf_struct_init id rem
    | Some bf ->
        let el, rem' = pack_bitfield_init id bf.bf_carrier fld_init_list in
        ( { fld_name= bf.bf_carrier
          ; fld_typ= bf.bf_carrier_typ
          ; fld_bitfield= None
          ; fld_anonymous= false }
        , Init_single
            { edesc= ECast (bf.bf_carrier_typ, or_expr_list el)
...
let rec is_bitfield_access env e =
  ...
  | EUnop (Odot fieldname, e1) -> (
    match unroll env e1.etyp with
    | TUnion (id, _) | TStruct (id, _) -> (
  ...
  | EUnop (Oarrow fieldname, e1) -> (
    ...
    | TPtr (ty, _) | TArray (ty, _, _) ->
        is_bitfield_access env
          { edesc= EUnop (Odot fieldname, {edesc= EUnop (Oderef, e1); etyp= ty})
          ; etyp= e.etyp }
    | _ -> None )
  | _ -> None
...
let rec transf_exp env ctx e =
  ...
  | EVar _ -> e
  | EUnop (Odot s, e1) -> (
    match is_bitfield_access env e with
    | None -> {edesc= EUnop (Odot s, transf_exp env Val e1); etyp= e.etyp}
    | Some (ex, bf) -> transf_read env ex bf )
  | EUnop (Oarrow s, e1) -> (
    match is_bitfield_access env e with
    | None -> {edesc= EUnop (Oarrow s, transf_exp env Val e1); etyp= e.etyp}
    | Some (ex, bf) -> transf_read env ex bf )
  | EUnop (((Opreincr | Opredecr) as op), e1) -> (
    match is_bitfield_access env e1 with
    | None -> {edesc= EUnop (op, transf_exp env Val e1); etyp= e.etyp}
    | Some (ex, bf) -> transf_pre env ctx (op_for_incr_decr op) ex bf e1.etyp )
  | EUnop (((Opostincr | Opostdecr) as op), e1) -> (
    match is_bitfield_access env e1 with
    | None -> {edesc= EUnop (op, transf_exp env Val e1); etyp= e.etyp}
    | Some (ex, bf) -> transf_post env ctx (op_for_incr_decr op) ex bf e1.etyp
    )
  | EUnop (op, e1) -> {edesc= EUnop (op, transf_exp env Val e1); etyp= e.etyp}
  | EBinop (Oassign, e1, e2, ty) -> (
    match is_bitfield_access env e1 with
    | None ->
        { edesc=
            EBinop (Oassign, transf_exp env Val e1, transf_exp env Val e2, ty)
        ; etyp= e.etyp }
    | Some (ex, bf) -> transf_assign env ctx ex bf e2 )
  | EBinop
    ...
    | None ->
        ...
        ; etyp= e.etyp }
    | Some (ex, bf) -> transf_assignop env ctx (op_for_assignop op) ex bf e2 ty
    )
  | EBinop (Ocomma, e1, e2, ty) ->
      { edesc=
          EBinop (Ocomma, transf_exp env Effects e1, transf_exp env Val e2, ty)
      ; etyp= e.etyp }
  | EBinop (op, e1, e2, ty) ->
      { edesc= EBinop (op, transf_exp env Val e1, transf_exp env Val e2, ty)
      ; etyp= e.etyp }
  | EConditional (e1, e2, e3) ->
      { edesc=
          EConditional
            ( transf_exp env Val e1
            , transf_exp env ctx e2
            , transf_exp env ctx e3 )
      ; etyp= e.etyp }
  | ECast (ty, e1) -> {edesc= ECast (ty, transf_exp env Val e1); etyp= e.etyp}
  | ECompound (ty, i) ->
...
and transf_assign env ctx e1 bf e2 =
  bind_lvalue env (transf_exp env Val e1) (fun base ->
      let carrier =
        {edesc= EUnop (Odot bf.bf_carrier, base); etyp= bf.bf_carrier_typ}
      in
      let asg =
        eassign carrier
          (bitfield_assign env bf carrier (transf_exp env Val e2))
      in
  ...
  bind_lvalue env (transf_exp env Val e1) (fun base ->
      ...
      let rhs =
        { edesc=
            EBinop
              ( op
              , bitfield_extract env bf carrier
              , transf_exp env Val e2
              , tyres )
        ; etyp= tyres }
      in
      let asg = eassign carrier (bitfield_assign env bf carrier rhs) in
      if ctx = Val then ecomma asg (bitfield_extract env bf carrier) else asg
  )
...
and transf_post env ctx op e1 bf tyfield =
  if ctx = Effects then transf_pre env ctx op e1 bf tyfield
  else
    bind_lvalue env (transf_exp env Val e1) (fun base ->
        let carrier =
          {edesc= EUnop (Odot bf.bf_carrier, base); etyp= bf.bf_carrier_typ}
        ...
        let rhs =
          {edesc= EBinop (op, temp, intconst 1L IInt, tyres); etyp= tyres}
        in
        let asg = eassign carrier (bitfield_assign env bf carrier rhs) in
...
and transf_init env i =
  ...
  | Init_struct (id, fld_init_list) ->
      let fld_init_list' =
        List.map (fun (f, i) -> (f, transf_init env i)) fld_init_list
      in
...
let transf_stmt env s =
  Transform.stmt
    ...
    ~decl:transf_decl env s

(* Functions *)

let transf_fundef env f = Transform.fundef transf_stmt env f

(* Programs *)

let program p =
  Transform.program ~composite:transf_composite ~decl:transf_decl
    ~fundef:transf_fundef p
