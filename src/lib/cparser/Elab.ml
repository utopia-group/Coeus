...
let fatal_error loc fmt = fatal_error (loc.filename, loc.lineno) fmt

let error loc fmt = error (loc.filename, loc.lineno) fmt

let warning loc = warning (loc.filename, loc.lineno)

let print_typ env fmt ty =
  match ty with
  | TNamed _ ->
      Format.fprintf fmt "'%a' (aka '%a')" Cprint.typ_raw ty Cprint.typ_raw
        (unroll env ty)
...
let wrap fn loc env arg =
  try fn env arg with Env.Error msg ->
    fatal_error loc "%s" (Env.error_message msg)

let wrap2 fn loc env arg1 arg2 =
  try fn env arg1 arg2 with Env.Error msg ->
    fatal_error loc "%s" (Env.error_message msg)
...
let reset () =
  top_declarations := [] ;
  top_environment := Env.empty ;
  global_defines := StringSet.empty
...
let previous_def fn env arg = try Some (fn env arg) with Env.Error _ -> None
...
let combine_toplevel_definitions loc env s old_sto old_ty sto ty =
  let new_ty =
    ...
    | Some new_ty -> new_ty
    | None ->
        error loc "redefinition of '%s' with a different type: %a vs %a" s
          (print_typ env) old_ty (print_typ env) ty ;
        ty
  in
  let new_sto =
    ...
    | _, Storage_register | Storage_register, _ -> Storage_register
  in
  (new_sto, new_ty)
...
let enter_or_refine_ident local loc env s sto ty =
  ...
  if local && sto <> Storage_extern then
    ...
    match
     ...
     The variable has linkage. *)
      previous_def Env.lookup_ident !top_environment s
    with
    | Some (id, Env.II_ident (old_sto, old_ty)) ->
        let new_sto, new_ty =
          combine_toplevel_definitions loc env s old_sto old_ty sto ty
        in
...
let integer_representable v ik =
  let bitsize = sizeof_ikind ik * 8 and signed = is_signed_ikind ik in
  if bitsize >= 64 then not signed || (v >= 0L && v <= 0x7FFF_FFFF_FFFF_FFFFL)
  else if not signed then v >= 0L && v < Int64.shift_left 1L bitsize
  else v >= 0L && v < Int64.shift_left 1L (bitsize - 1)
...
let elab_int_constant loc s0 =
  let s =
    String.map
      (fun d ->
        ...
        | _ ->
            error loc "bad digit '%c' in integer literal '%s'" d s0 ;
            d )
      s0
  in
  ...
  let s, dec_kinds, hex_kinds =
    if has_suffix s "ULL" || has_suffix s "LLU" then
      ...
      (chop_last s 1, [IUInt; IULong; IULongLong], [IUInt; IULong; IULongLong])
    else
      ( s
      , [IInt; ILong; ILongLong]
      , [IInt; IUInt; ILong; IULong; ILongLong; IULongLong] )
  in
  (* Determine base *)
  let s, base =
    if has_prefix s "0X" then (chop_first s 2, 16)
    else if has_prefix s "0" then (chop_first s 1, 8)
    else (s, 10)
  in
  (* Parse digits *)
  let v =
    try parse_int base s with
    | Overflow ->
        error loc
          "integer literal '%s' is too large to be represented in any integer \
           type"
          s0 ;
        0L
    | Bad_digit ->
  ...
  let ty =
    try
      List.find
        (fun ty -> integer_representable v ty)
...
let elab_float_constant f =
  let ty =
    match f.suffix_FI with
    | Some ("l" | "L") -> FLongDouble
    | Some ("f" | "F") -> FFloat
    | None -> FDouble
    | _ -> assert false
    (* The lexer should not accept anything else. *)
  in
  let v =
    { hex= f.isHex_FI
    ; intPart= (match f.integer_FI with Some s -> s | None -> "0")
    ; fracPart= (match f.fraction_FI with Some s -> s | None -> "0")
    ; exp= (match f.exponent_FI with Some s -> s | None -> "0") }
  in
  (v, ty)
...
let elab_char_constant loc wide chars =
  ...
  Ceval.normalize_int v
    ( if List.length chars = 1 then if wide then wchar_ikind () else IChar
    else IInt )
...
let elab_string_literal loc wide chars =
  ...
  List.iter
    (fun c ->
      if c < 0L || c >= char_max then
        error loc "escape sequence is out of range (code 0x%LX)" c )
    chars ;
  if wide then CWStr chars
  else
...
let elab_constant loc = function
  ...
  | CONST_FLOAT f ->
      ...
      CFloat (v, fk)
  | CONST_CHAR (wide, s) -> CInt (elab_char_constant loc wide s, IInt, "")
  | CONST_STRING (wide, s) -> elab_string_literal loc wide s

let elab_simple_string loc wide chars =
  match elab_string_literal loc wide chars with
  | CStr s -> s
  | _ ->
      error loc "cannot use wide string literal in 'asm'" ;
      ""
...
let elab_gcc_attr loc env = function
  ...
  | GCC_ATTR_ARGS (w, args) ->
      let v = elab_gcc_attr_word w in
      try [Attr (v, List.map (elab_attr_arg loc env) args)]
      with Wrong_attr_arg ->
        warning loc Unknown_attribute "unknown attribute '%s' ignored" v ;
        []
...
let extract_alignas loc a =
  ...
  | Attr (("aligned" | "__aligned__"), args) -> (
    ...
    | [AInt n] ->
        ...
        a
    | [_] ->
        error loc "requested alignment is not an integer constant" ;
        a
    | [] -> a (* Use the default alignment as the gcc does *)
    | _ ->
        error loc "'aligned' attribute takes no more than 1 argument" ;
        a )
  | _ -> a
...
let elab_attribute env = function
  ...
  | ALIGNAS_ATTR ([a], loc) -> (
    match elab_attr_arg loc env a with
    | AInt n ->
        if is_power_of_two n then [AAlignas (Int64.to_int n)]
        else (
          error loc "requested alignment is not a power of 2" ;
          [] )
    | _ ->
...
(* There should be at most one of the others *)
...
let rec elab_specifier keep_ty ?(only= false) loc env specifier =
  ...
  let do_specifier = function
    | SpecCV cv -> attr := add_attributes (elab_cvspec env cv) !attr
    | SpecStorage st -> (
        ...
        | TYPEDEF ->
            if !typedef then error loc "multiple uses of 'typedef'" ;
            typedef := true )
  ...
  let simple ty =
    (!sto, !inline, !noreturn, !typedef, add_attributes_type !attr ty, env)
  in
  ...
  let get_struct_attrs () =
    let ta, nta =
      List.partition (fun a -> class_of_attribute a = Attr_struct) !attr
    in
    attr := nta ;
    ta
  in
  ...
  | [Cabs.Tstruct_union (STRUCT, id, optmembers, a)] ->
      let a' = add_attributes (get_struct_attrs ()) (elab_attributes env a) in
      let id', env' =
        elab_struct_or_union keep_ty only Struct loc id optmembers a' env
      in
      (!sto, !inline, !noreturn, !typedef, TStruct (id', !attr), env')
  | [Cabs.Tstruct_union (UNION, id, optmembers, a)] ->
      let a' = add_attributes (get_struct_attrs ()) (elab_attributes env a) in
      let id', env' =
        elab_struct_or_union keep_ty only Union loc id optmembers a' env
      in
      (!sto, !inline, !noreturn, !typedef, TUnion (id', !attr), env')
  | [Cabs.Tenum (id, optmembers, a)] ->
      let a' = add_attributes (get_struct_attrs ()) (elab_attributes env a) in
      let id', env' = elab_enum only loc id optmembers a' env in
      (!sto, !inline, !noreturn, !typedef, TEnum (id', !attr), env')
  (* Specifier doesn't make sense *)
  | _ -> fatal_error loc "illegal combination of type specifiers"
...
and elab_type_declarator keep_ty loc env ty kr_ok = function
      ...
      let sz' =
        ...
        | Some sz ->
            ...
            | Some n ->
                ...
                if n = 0L then
                  warning loc Zero_length_array
                    "zero size arrays are an extension" ;
                if not (Cutil.valid_array_size env ty n) then
                  error loc "size of array is too large" ;
                Some n
            | None ->
                error loc "size of array is not a compile-time constant" ;
                Some 1L
      in
      (* produces better error messages later *)
  ...
  | Cabs.PROTO (d, (params, vararg)) ->
      ...
      elab_type_declarator keep_ty loc env (TFun (ty, Some params', vararg, a))
        kr_ok d
  | Cabs.PROTO_OLD (d, params) ->
      ...
      match params with
      | [] ->
          elab_type_declarator keep_ty loc env (TFun (ty, None, false, a))
            kr_ok d
      | _ ->
...
and elab_parameter keep_ty env (PARAM (spec, id, decl, attr, loc)) =
  let sto, inl, noret, tydef, bty, env1 =
    elab_specifier keep_ty loc env spec
  in
  if tydef then error loc "'typedef' used in function parameter" ;
  ...
  if inl then error loc "'inline' can only appear on functions" ;
  if noret then error loc "'_Noreturn' can only appear on functions" ;
...
and elab_fundef_name keep_ty env spec (Name (id, decl, attr, loc)) =
  let sto, inl, noret, tydef, bty, env' =
    elab_specifier keep_ty loc env spec
  in
  if tydef then error loc "'typedef' is forbidden here" ;
  let (ty, kr_params), env'' =
    elab_type_declarator keep_ty loc env' bty true decl
  in
...
and elab_name_group keep_ty loc env (spec, namelist) =
  ...
  in
  if tydef then error loc "'typedef' is forbidden here" ;
  if inl then error loc "'inline' is forbidden here" ;
  if noret then error loc "'_Noreturn' is forbidden here" ;
  let elab_one_name env (Name (id, decl, attr, loc)) =
    let (ty, _), env1 = elab_type_declarator keep_ty loc env bty false decl in
    let a = elab_attributes env attr in
    ((id, add_attributes_type a ty), env1)
  in
...
and elab_init_name_group keep_ty loc env (spec, namelist) =
  let sto, inl, noret, tydef, bty, env' =
    elab_specifier keep_ty ~only:(namelist = []) loc env spec
  in
  let elab_one_name env (Init_name (Name (id, decl, attr, loc), init)) =
    ...
    in
    ((id, add_attributes_type a' ty, init), env1)
  in
...
and elab_field_group keep_ty env (Field_group (spec, fieldlist, loc)) =
  let fieldlist =
    List.map
      (function
          | None, x -> (Name ("", JUSTBASE, [], loc), x) | Some n, x -> (n, x))
      fieldlist
  in
  let (names, env'), sto =
    elab_name_group keep_ty loc env (spec, List.map fst fieldlist)
  in
    ...
    let optbitsize', env' =
      ...
      | Some sz ->
          let ik =
            ...
            | _ -> ILongLong
            (* trigger next error message *)
          in
          if integer_rank ik > integer_rank IInt then (
            error loc
              "the type of bit-field '%a' must be an integer type no bigger \
               than 'int'"
              pp_field id ;
            (None, env) )
          else
            ...
            | Some n ->
                if n < 0L then (
                  error loc "bit-field '%a' has negative width (%Ld)" pp_field
                    id n ;
                  (None, env) )
                else
                  ...
                  if n > max then (
                    error loc
                      "size of bit-field '%a' (%Ld bits) exceeds its type \
                       (%Ld bits)"
                      pp_field id n max ;
                    (None, env) )
                  else if n = 0L && id <> "" then (
                    error loc "named bit-field '%a' has zero width" pp_field id ;
                    (None, env) )
                  else (Some (Int64.to_int n), env')
            | None ->
                error loc "bit-field '%a' width not an integer constant"
                  pp_field id ;
                (None, env)
    in
...
and elab_struct_or_union_info keep_ty kind loc env members attrs =
  ...
  let rec check_incomplete = function
    ...
    | fld :: rem ->
        if wrap incomplete_type loc env' fld.fld_typ then
          ...
          fatal_error loc "member '%a' has incomplete type" pp_field
            fld.fld_name ;
        check_incomplete rem
  in
  check_incomplete m ;
  ...
  if m = [] then
    ...
    else fatal_error loc "empty union is a GNU extension" ;
  (composite_info_def env' kind attrs m, env')

and elab_struct_or_union keep_ty only kind loc tag optmembers attrs env =
  let warn_attrs () =
    if attrs <> [] then
      warning loc Unnamed "attribute declaration must precede definition"
  in
  let optbinding, tag =
    match tag with
    | None -> (None, "")
    | Some s -> (Env.lookup_composite env s, s)
  in
  match (optbinding, optmembers) with
  | Some (tag', ci), None when not only || Env.in_current_scope env tag' ->
  ...
  | Some (tag', ({Env.ci_sizeof= None} as ci)), Some members
    when Env.in_current_scope env tag' ->
      ...
      let ci', env' =
        elab_struct_or_union_info keep_ty kind loc env members attrs
      in
      (* Emit a global definition for it *)
      emit_elab env' loc
        (Gcompositedef (kind, tag', attrs, ci'.Env.ci_members)) ;
  ...
  | _, None ->
      ...
      if tag = "" then error loc "anonymous, incomplete struct or union" ;
  ...
  | _, Some members ->
      ...
      (* emit a definition *)
      emit_elab env'' loc
        (Gcompositedef (kind, tag', attrs, ci2.Env.ci_members)) ;
      (* Replace infos but keep same ident *)
...
and elab_enum_item env ((s, exp), loc) nextval =
  let v, exp' =
    ...
    | Some exp ->
        ...
        | Some n -> (n, Some exp')
        | None ->
            error loc "value of enumerator '%s' is not an integer constant" s ;
            (nextval, Some exp')
  in
  ...
  if
    not
      (int_representable v
         (8 * sizeof_ikind enum_ikind)
         (is_signed_ikind enum_ikind))
  then
    warning loc Constant_conversion
      "integer literal '%Ld' is too large to be represented in any integer type"
      v ;
...
and elab_enum only loc tag optmembers attrs env =
  ...
  | None ->
      ...
      let tag', info = wrap Env.lookup_enum loc env tag in
      (tag', env)
  | Some members ->
...
let elab_type loc env spec decl =
  ...
  if sto <> Storage_default || inl || noret || tydef then
    error loc
      "'typedef', 'extern', 'static', 'register' and 'inline' are meaningless \
       in cast" ;
  (ty, env'')
...
let init_char_array_string opt_size s =
  ...
  let size =
    match opt_size with Some sz -> sz | None -> Int64.succ len
    (* include final 0 character *)
  in
  let rec add_chars i init =
    if i < 0L then init
    else
      let c =
        if i < len then Int64.of_int (Char.code s.[Int64.to_int i]) else 0L
      in
  ...
  if wrap2 valid_assignment loc env a ty then ()
    ...
    if wrap2 int_pointer_conversion loc env a.etyp ty then
      warning loc Int_conversion
        "incompatible integer-pointer conversion initializer has type %a \
         instead of the expected type %a"
        (print_typ env) a.etyp (print_typ env) ty
    else
      warning loc Unnamed
        "incompatible conversion initializer has type %a instead of the \
         expected type %a"
        (print_typ env) a.etyp (print_typ env) ty
  else
    error loc "initializer has type %a instead of the expected type %a"
      (print_typ env) a.etyp (print_typ env) ty
...
module I = struct
  type zipinit =
    ...
    | Zarray of
        ...
        * init list
        (* elements before point, reversed *)
        * int64 (* position of point *)
        * init list
    (* elements after point *)
    | Zstruct of
        zipinit (* ancestor *)
        * ident (* struct type *)
        * (field * init) list
        (* elements before current, reversed *)
        * field (* current field *)
        * (field * init) list
    (* elements after current *)
    | Zunion of zipinit (* ancestor *) * ident (* union type *) * field

  (* current member *)

  type 'a result = OK of 'a | NotFound | Unsupported
  ...
  let rec to_top = function
    ...
    | Zunion (z, id, fld), i -> to_top (z, Init_union (id, fld, i))
  ...
  let rec zipname = function
    ...
    | Zunion (z, id, fld) -> Printf.sprintf "%s.%s" (zipname z) fld.fld_name
  ...
  let rec next = function
    ...
    | Zarray (z, ty, sz, dfl, before, idx, after), i ->
        ...
        if index_below idx' sz then
          OK
            ( Zarray (z, ty, sz, dfl, i :: before, idx', il_tail after)
            , il_head dfl after )
    ...
    | Zunion (z, id, fld), i -> next (z, Init_union (id, fld, i))
  ...
  let index env ((z, i) as zi) n =
    ...
    | TArray (ty, sz, _), Init_array il ->
        if n >= 0L && index_below n sz then
          let dfl = default_init env ty in
          let rec loop p before after =
            if p = n then
              OK
                ( Zarray (z, ty, sz, dfl, before, n, il_tail after)
                , il_head dfl after )
            else
              loop (Int64.succ p) (il_head dfl after :: before) (il_tail after)
          in
          loop 0L [] il
        else NotFound
    | _, _ -> NotFound
  ...
  let has_member env name ty =
    let mem f id =
      try
        ignore (f env (id, name)) ;
        true
      with Env.Error _ -> false
    in
    match ty with
    | TStruct (id, _) -> mem Env.find_struct_member id
    | TUnion (id, _) -> mem Env.find_union_member id
    | _ -> false
        ...
        let rec find before = function
          ...
          | ((fld, i) as f_i) :: after ->
              if fld.fld_name = name then
                ...
                member env zi name
              else find (f_i :: before) after
        in
        find [] flds
    | TUnion (id, _), Init_union (id', fld, i) ->
        if fld.fld_name = name then OK (Zunion (z, id, fld), i)
        else
          let rec find = function
            ...
            | fld1 :: rem ->
                ...
                else if fld.fld_anonymous && has_member env name fld.fld_typ
                then
                  let zi = (Zunion (z, id, fld1), i) in
                  member env zi name
                else find rem
          in
          find (Env.find_union env id).Env.ci_members
    | (TStruct _ | TUnion _), Init_single a ->
        (* This is a previous whole-struct initialization that we
           are going to overwrite.  Hard to support correctly
           and without code duplication, so turn it into an error. *)
...
let rec elab_designator loc env zi desig =
  ...
  | INFIELD_INIT name :: desig' -> (
    match I.member env zi name with
    | I.OK zi' -> elab_designator loc env zi' desig'
    | I.NotFound ->
        error loc "%s has no member named %s" (I.name zi) name ;
        raise Exit
    | I.Unsupported ->
        error loc
          "unsupported reinitialization of %s that was previously initialized \
           as a whole"
          (I.name zi) ;
        raise Exit )
  | ATINDEX_INIT a :: desig' ->
      ...
      | None ->
          error loc
            "array element designator for %s is not an integer constant \
             expression"
            (I.name zi) ;
          raise Exit
      | Some n ->
        ...
        | I.OK zi' -> elab_designator loc env zi' desig'
        | I.NotFound ->
            error loc "array index %Ld within %s exceeds array bounds" n
              (I.name zi) ;
            raise Exit
...
let elab_init loc env root ty_root ie =
  ...
  let rec elab_list zi il first =
    ...
    | (desig, item) :: il' ->
        if desig = [] then (
          match if first then I.first env zi else I.next zi with
          | I.NotFound ->
              warning loc Unnamed "excess elements in initializer for %s"
                (I.name zi) ;
              I.to_init zi
          | I.OK zi' -> elab_item zi' item il'
          | I.Unsupported ->
              error loc
                "unsupported reinitialization of %s that was previously \
                 initialized as a whole"
                (I.name zi) ;
              raise Exit )
        else elab_item (elab_designator loc env (I.to_top zi) desig) item il'
  ...
  and elab_item zi item il =
    ...
    | ( ( SINGLE_INIT (CONSTANT (CONST_STRING (w, s)))
      ...
      match (elab_string_literal loc w s, unroll env ty_elt) with
      ...
      | CStr _, _ ->
          error loc
            "initialization of an array of non-char elements with a string \
             literal" ;
          elab_list zi il false
      | CWStr s, TInt (_, _)
        when compatible_types AttrIgnoreTop env ty_elt
               (TInt (wchar_ikind (), [])) ->
          if not (I.index_below (Int64.of_int (List.length s - 1)) sz) then
            warning loc Unnamed
              "initializer string for array of wide chars %s is too long"
              (I.name zi) ;
          elab_list (I.set zi (init_int_array_wstring sz s)) il false
      | CWStr _, _ ->
          error loc
            "initialization of an array of non-wchar_t elements with a wide \
             string literal" ;
          elab_list zi il false
      | _ -> assert false )
    ...
    | NO_INIT, _ -> elab_list zi il false
  ...
  and elab_single zi a il =
    ...
    | TStruct _ | TUnion _ | TArray _ -> (
        ...
        match I.first env zi with
        | I.OK zi' -> elab_single zi' a il
        | I.NotFound ->
            error loc
              "initializer for aggregate %s with no elements requires \
               explicit braces"
              (I.name zi) ;
            raise Exit
        | I.Unsupported ->
            ...
            raise Exit )
    | _ ->
        error loc "impossible to initialize %s of type %a" (I.name zi)
          (print_typ env) ty ;
        raise Exit
    (* Start with top-level object initialized to default *)
  in
  if is_function_type env ty_root then (
    error loc "illegal initializer (only variables can be initialized)" ;
    raise Exit ) ;
  try elab_item (I.top env root ty_root) ie [] with No_default_init ->
...
let elab_initial loc env root ty ie =
  ...
  | NO_INIT -> None
  | _ ->
    try Some (elab_init loc env root ty ie) with
    | Exit -> None (* error was already reported *)
    | Env.Error msg ->
        error loc "%s" (Env.error_message msg) ;
        None
...
let fixup_typ loc env ty init =
  ...
  | TArray (ty_elt, None, attr), Init_array il ->
      if il = [] then
        warning loc Zero_length_array "zero size arrays are an extension" ;
...
let elab_initializer loc env root ty ie =
  ...
  | None -> (ty, None)
  | Some init -> (fixup_typ loc env ty init, Some init)

(* Elaboration of expressions *)

let elab_expr vararg loc env a =
  let err fmt = error loc fmt in
  (* non-fatal error *)
  let error fmt = fatal_error loc fmt in
  let warning t fmt = warning loc t fmt in
  let check_ptr_arith env ty s =
    match unroll env ty with
    | TVoid _ -> err "illegal arithmetic on a pointer to void in binary '%c'" s
    | TFun _ ->
        err
          "illegal arithmetic on a pointer to the function type %a in binary \
           '%c'"
          (print_typ env) ty s
    | _ ->
        if incomplete_type env ty then
          err "arithmetic on a pointer to an incomplete type %a in binary '%c'"
            (print_typ env) ty s
  in
  let rec elab env = function
    (* 6.5.1 Primary expressions *)
    | VARIABLE s -> (
      match wrap Env.lookup_ident loc env s with
      | id, Env.II_ident (sto, ty) -> ({edesc= EVar id; etyp= ty}, env)
      | id, Env.II_enum v ->
    ...
    | INDEX (a1, a2) ->
        ...
        let tres =
          ...
          | t1, t2 -> error "subscripted value is neither an array nor pointer"
        in
    ...
    | MEMBEROF (a1, fieldname) ->
        ...
        let rec access = function
          ...
          | fld :: rest ->
              ...
              ; etyp=
                  add_attributes_type
                    (List.filter attr_inherited_by_members attrs)
          ...
          | TPtr (t, _) | TArray (t, _, _) -> (
            ...
            | _ ->
                error
                  ...
                  fieldname )
          | _ ->
              error "member reference type %a is not a pointer" (print_typ env)
                b1.etyp
        in
        let rec access = function
          | [] -> assert false
...
*)
    | CALL ((VARIABLE "__builtin_va_start" as a1), [a2; a3]) ->
        if not vararg then err "'va_start' used in function with fixed args" ;
        let b1, env = elab env a1 in
        let b2, env = elab env a2 in
        let _b3, env = elab env a3 in
        ({edesc= ECall (b1, [b2]); etyp= TVoid []}, env)
    | BUILTIN_VA_ARG (a2, a3) ->
        ...
        if not (compatible_types AttrIgnoreTop env ty ty') then
          warning Varargs
            "%a is promoted to %a when passed through '...'. You should pass \
             %a, not %a, to 'va_arg'"
            (print_typ env) ty (print_typ env) ty' (print_typ env) ty'
            (print_typ env) ty ;
    ...
    | CALL (a1, al) ->
        let b1, env =
          ...
          | VARIABLE n when not (Env.ident_is_bound env n) ->
              warning Implicit_function_declaration
                "implicit declaration of function '%s' is invalid in C99" n ;
              let ty = TFun (TInt (IInt, []), None, false, []) in
              (* Check against other definitions and enter in env *)
              let id, sto, env, ty, linkage =
                enter_or_refine_ident true loc env n Storage_extern ty
              in
        ...
        let res, args, vararg =
          ...
          | TPtr (ty, a) -> (
            ...
            | _ ->
                error
                  "called object type %a is neither a function nor function \
                   pointer"
        ...
        let bl', env =
          match args with
          | None -> bl
          | Some proto -> elab_arguments 1 bl proto vararg
        in
        ({edesc= ECall (b1, bl'); etyp= res}, env)
    | UNARY (POSINCR, a1) -> elab_pre_post_incr_decr Opostincr "increment" a1
    | UNARY (POSDECR, a1) -> elab_pre_post_incr_decr Opostdecr "decrement" a1
    (* 6.5.4 Cast operators *)
    | CAST ((spec, dcl), SINGLE_INIT a1) ->
        let ty, env = elab_type loc env spec dcl in
        let b1, env = elab env a1 in
        ( if not (wrap2 valid_cast loc env b1.etyp ty) then
            match (unroll env b1.etyp, unroll env ty) with
            | _, (TStruct _ | TUnion _ | TVoid _) ->
                err "used type %a where arithmetic or pointer type is required"
                  (print_typ env) ty
            | (TStruct _ | TUnion _ | TVoid _), _ ->
                err
                  "operand of type %a where arithmetic or pointer type is \
                   required"
                  (print_typ env) b1.etyp
            | TFloat _, TPtr _ ->
            ...
            | _ ->
                err "illegal cast from %a to %a" (print_typ env) b1.etyp
                  (print_typ env) ty ) ;
        ({edesc= ECast (ty, b1); etyp= ty}, env)
    (* 6.5.2.5 Compound literals *)
    | CAST ((spec, dcl), ie) -> (
        let ty, env = elab_type loc env spec dcl in
        match elab_initializer loc env "<compound literal>" ty ie with
    ...
    | EXPR_ALIGNOF a1 ->
        let b1, env = elab env a1 in
        if wrap incomplete_type loc env b1.etyp then
          error "invalid application of '_Alignof' to an incomplete type %a"
            (print_typ env) b1.etyp ;
        if wrap is_bitfield loc env b1 then
    ...
    | TYPE_ALIGNOF (spec, dcl) ->
        let ty, env' = elab_type loc env spec dcl in
        if wrap incomplete_type loc env' ty then
          error "invalid application of 'alignof' to an incomplete type %a"
            (print_typ env) ty ;
    ...
    | BUILTIN_OFFSETOF ((spec, dcl), mem) ->
        ...
        let rec offset_of_list acc env ty = function
          ...
          | fld :: rest ->
              ...
              offset_of_list (acc + off) env fld.fld_typ rest
        in
        let offset_of_member (env, off_accu, ty) mem =
          ...
          | ATINDEX_INIT e, TArray (sub_ty, _, _) ->
              ...
              let e =
                ...
                | None ->
                    error
                      "array element designator for is not an integer \
                       constant expression"
                | Some n -> n
              in
              let size =
                match sizeof env sub_ty with
                | None -> assert false (* We expect only complete types *)
                | Some s -> s
              in
              let off_accu =
                match cautious_mul e size with
                | None -> error "'offsetof' overflows"
                | Some s -> off_accu + s
              in
              (env, off_accu, sub_ty)
          | ATINDEX_INIT _, _ -> error "subscripted value is not an array"
        in
        let env, offset, _ =
          List.fold_left offset_of_member (env, 0, ty) mem
        in
    ...
    | UNARY (ADDROF, a1) ->
        ...
        if not (is_lvalue b1 || is_function_type env b1.etyp) then
          err "argument of '&' is not an lvalue (invalid %a)" (print_typ env)
            b1.etyp ;
        ( match b1.edesc with
        | EVar id -> (
          match wrap Env.find_ident loc env id with
    ...
    | UNARY (MEMOF, a1) -> (
        ...
        | _ ->
            error "arguemnt of unary '*' is not a pointer (%a invalid)"
              (print_typ env) b1.etyp )
    | UNARY (PREINCR, a1) -> elab_pre_post_incr_decr Opreincr "increment" a1
    | UNARY (PREDECR, a1) -> elab_pre_post_incr_decr Opredecr "decrement" a1
    (* 6.5.5 to 6.5.12  Binary operator expressions *)
    | BINARY (MUL, a1, a2) ->
        elab_binary_arithmetic "*" Omul a1 a2
    | BINARY (DIV, a1, a2) -> elab_binary_arithmetic "/" Odiv a1 a2
    | BINARY (MOD, a1, a2) -> elab_binary_integer "%" Omod a1 a2
    | BINARY (ADD, a1, a2) ->
        let b1, env = elab env a1 in
        let b2, env = elab env a2 in
        let tyres =
          if is_arith_type env b1.etyp && is_arith_type env b2.etyp then
            ...
            let ty =
              ...
              | _, _ ->
                  error "invalid operands to binary '+' (%a and %a)"
                    (print_typ env) b1.etyp (print_typ env) b2.etyp
            in
            check_ptr_arith env ty '+' ; TPtr (ty, [])
        in
    ...
    | BINARY (SUB, a1, a2) ->
        ...
        let tyop, tyres =
          if is_arith_type env b1.etyp && is_arith_type env b2.etyp then
            ...
            match
              (wrap unroll loc env b1.etyp, wrap unroll loc env b2.etyp)
            with
    ...
    | BINARY (SHL, a1, a2) -> elab_shift "<<" Oshl a1 a2
    | BINARY (SHR, a1, a2) -> elab_shift ">>" Oshr a1 a2
    | BINARY (EQ, a1, a2) -> elab_comparison Oeq a1 a2
    | BINARY (NE, a1, a2) -> elab_comparison One a1 a2
    | BINARY (LT, a1, a2) -> elab_comparison Olt a1 a2
    | BINARY (GT, a1, a2) -> elab_comparison Ogt a1 a2
    | BINARY (LE, a1, a2) -> elab_comparison Ole a1 a2
    | BINARY (GE, a1, a2) -> elab_comparison Oge a1 a2
    | BINARY (BAND, a1, a2) -> elab_binary_integer "&" Oand a1 a2
    | BINARY (BOR, a1, a2) -> elab_binary_integer "|" Oor a1 a2
    | BINARY (XOR, a1, a2) -> elab_binary_integer "^" Oxor a1 a2
    (* 6.5.13 and 6.5.14 Logical operator expressions *)
    | BINARY (AND, a1, a2) ->
        elab_logical_operator "&&" Ologand a1 a2
    | BINARY (OR, a1, a2) -> elab_logical_operator "||" Ologor a1 a2
    ...
    | QUESTION (a1, a2, a3) -> (
        ...
        | (TPtr (ty1, a1) as pty1), (TPtr (ty2, a2) as pty2) ->
            let tyres =
              if is_void_type env ty1 || is_void_type env ty2 then
                ...
                match
                  combine_types AttrIgnoreAll env (TPtr (ty1, a1))
                    (TPtr (ty2, a2))
                with
                | None ->
                    warning Pointer_type_mismatch
                      "the second and third argument of '?:' have \
        ...
        | ty1, ty2 ->
          ...
          | None ->
              error
                "the second and third argument of '?:' incompatible types (%a \
                 and %a)"
                (print_typ env) ty1 (print_typ env) ty2
          | Some tyres -> ({edesc= EConditional (b1, b2, b3); etyp= tyres}, env)
        )
    ...
    | BINARY (ASSIGN, a1, a2) ->
        ...
        if not (wrap2 valid_assignment loc env b2 b1.etyp) then
          if wrap2 valid_cast loc env b2.etyp b1.etyp then
            if wrap2 int_pointer_conversion loc env b2.etyp b1.etyp then
              warning Int_conversion
                "incompatible integer-pointer conversion: assigning to %a \
                 from %a"
                (print_typ env) b1.etyp (print_typ env) b2.etyp
            else
              warning Unnamed "incompatible conversion assigning to %a from %a"
                (print_typ env) b1.etyp (print_typ env) b2.etyp
          else
            err "assigning to %a from incompatible type %a" (print_typ env)
              b1.etyp (print_typ env) b2.etyp ;
    ...
    | BINARY
      ...
      -> (
        let sop, top =
          ...
          | _ -> assert false
        in
        match elab env (BINARY (sop, a1, a2)) with
        | ({edesc= EBinop (_, b1, b2, _); etyp= ty} as b), env ->
            if List.mem AConst (attributes_of_type env b1.etyp) then
              err "left-hand side of assignment has 'const' type" ;
            if not (is_modifiable_lvalue env b1) then
              err "expression is not assignable" ;
            if not (wrap2 valid_assignment loc env b b1.etyp) then
  ...
  and elab_binary_integer msg op a1 a2 =
    ...
    if not (is_integer_type env b1.etyp && is_integer_type env b2.etyp) then
      error "invalid operands to binary '%s' (%a and %a)" msg (print_typ env)
        b1.etyp (print_typ env) b2.etyp ;
    let tyres = binary_conversion env b1.etyp b2.etyp in
  ...
  and elab_shift msg op a1 a2 =
    ...
    let b2, env = elab env a2 in
    if not (is_integer_type env b1.etyp && is_integer_type env b2.etyp) then
      error "invalid operands to '%s' (%a and %a)" msg (print_typ env) b1.etyp
        (print_typ env) b2.etyp ;
  ...
  and elab_comparison op a1 a2 =
    ...
    let resdesc =
      ...
      | TPtr (ty1, _), TPtr (ty2, _) when is_void_type env ty1 ->
          EBinop (op, b1, b2, TPtr (ty2, []))
      | TPtr (ty1, _), TPtr (ty2, _) when is_void_type env ty2 ->
          EBinop (op, b1, b2, TPtr (ty1, []))
      | TPtr (ty1, _), TPtr (ty2, _) ->
          if not (compatible_types AttrIgnoreAll env ty1 ty2) then
            warning Compare_distinct_pointer_types
              "comparison of distinct pointer types (%a and %a)"
              (print_typ env) b1.etyp (print_typ env) b2.etyp ;
          EBinop (op, b1, b2, TPtr (ty1, []))
      | TPtr _, (TInt _ | TEnum _) | (TInt _ | TEnum _), TPtr _ ->
          warning Unnamed "comparison between pointer and integer (%a and %a)"
            (print_typ env) b1.etyp (print_typ env) b2.etyp ;
          EBinop (op, b1, b2, TPtr (TVoid [], []))
      | ty1, ty2 ->
          error "illegal comparison between types@ %a@ and %a" (print_typ env)
            b1.etyp (print_typ env) b2.etyp
    in
  ...
  and elab_arguments argno args params vararg =
    ...
    | ([], env), _ :: _ ->
        ...
        err "too few arguments to function call, expected %s%d, have %d" vararg
          expected found ;
        ([], env)
    | (_ :: _, env), [] ->
        if vararg then args
          ...
          err "too many arguments to function call, expected %d, have %d"
            expected found ;
          args
        ...
        then
          if wrap2 valid_cast loc env ty_a ty_p then
            if wrap2 int_pointer_conversion loc env ty_a ty_p then
              warning Int_conversion
                "incompatible integer-pointer conversion: passing %a to \
                 parameter %d of type %a"
            ...
            err "passing %a to parameter %d of incompatible type %a"
              (print_typ env) ty_a argno (print_typ env) ty_p ;
...
let elab_opt_expr vararg loc env = function
  | None -> (None, env)
  | Some a ->
      let a, env = elab_expr vararg loc env a in
      (Some a, env)
...
let __func__type_and_init s =
  ( TArray
      (TInt (IChar, [AConst]), Some (Int64.of_int (String.length s + 1)), [])
...
let enter_typedefs loc env sto dl =
  ...
  List.fold_left
    (fun env (s, ty, init) ->
      if init <> NO_INIT then error loc "initializer in typedef" ;
      match previous_def Env.lookup_typedef env s with
      | Some (s', ty') ->
          if equal_types env ty ty' then (
            warning loc Celeven_extension
              "redefinition of typedef '%s' is C11 extension" s ;
            env )
          else (
...
let enter_decdefs local loc env sto dl =
  ...
  let enter_decdef (decls, env) (s, ty, init) =
    ...
    ( if local && isfun then
        ...
        | Storage_static ->
            error loc
              "function declared in block scope cannot have 'static' \
               storage-class"
        | Storage_register ->
            error loc "invalid 'register' storage-class on function"
        | _ -> () ) ;
    ...
    if init <> NO_INIT && not local then add_global_define loc s ;
    let id, sto', env1, ty, linkage =
      enter_or_refine_ident local loc env s sto1 ty
    in
    ...
    if
      local && sto' <> Storage_extern && not isfun
      && wrap incomplete_type loc env ty'
    then error loc "variable has incomplete type %a" (print_typ env) ty' ;
    if local && not isfun && sto' <> Storage_extern && sto' <> Storage_static
...
let elab_KR_function_parameters env params defs loc =
  ...
  let elab_param_def env = function
    | DECDEF ((spec', name_init_list), loc') ->
        ...
        let paramsenv, sto =
          elab_name_group true loc' env (spec', name_list)
        in
        ( match sto with
        | Storage_extern ->
            error loc'
              "invalid 'extern' storage-class specifier for function parameter"
        | Storage_static ->
            error loc'
              "invalid 'static' storage-class specifier for function parameter"
        | _ -> () ) ;
        paramsenv
    | d ->
        (* Should never be produced by the parser *)
        fatal_error
          (Cabshelper.get_definitionloc d)
          "Illegal declaration of function parameter"
  in
  let kr_params_defs, paramsenv =
    let params, paramsenv = mmap elab_param_def env defs in
    (List.concat params, paramsenv)
  in
  ...
  let type_of_param param =
    ...
    | [] ->
        ...
        TInt (IInt, [])
    | (_, ty) :: rem ->
        if rem <> [] then error loc "redefinition of parameter '%s'" param ;
        ty
  in
  ...
  let rec match_params params' extra_decls = function
    ...
    | p :: ps ->
        ...
        if compatible_types AttrIgnoreTop env ty_var ty_param then
          ...
          match_params ((id_param, ty_param) :: params')
            ((Storage_default, id_var, ty_var, Some init) :: extra_decls) ps
  in
...
let elab_fundef env spec name defs body loc =
  ...
  let ty, extra_decls, env1 =
    ...
    | TFun (ty_ret, None, false, attr), Some params ->
        warning loc CompCert_conformance
          "non-prototype, pre-standard function definition, converting to \
           prototype form" ;
        let params', extra_decls, env =
          elab_KR_function_parameters env params defs loc
        in
        ( TFun (ty_ret, Some params', inherit_vararg env s sto ty, attr)
        , extra_decls
        , env )
  ...
  let env2 =
    List.fold_left
      (fun e (id, ty) ->
        incomplete_param e ty ;
        Env.add_ident e id Storage_default ty )
      (Env.new_scope env1) params
  in
  let env2 =
    List.fold_left
      (fun e (sto, id, ty, init) -> Env.add_ident e id sto ty)
      env2 extra_decls
  in
  ...
  let func_id, _, env3, func_ty, _ =
    enter_or_refine_ident true loc env2 "__func__" Storage_static func_ty
  in
  ...
  let body2 =
    if s = "main" then (
      if inline then error loc "'main' is not allowed to be declared inline" ;
      if noret then
  ...
  let body3 =
    if extra_decls = [] then body2
    else
      let mkdecl d = {sdesc= Sdecl d; sloc= no_loc} in
      {sdesc= Sblock (List.map mkdecl extra_decls @ [body2]); sloc= no_loc}
  in
  (* Handling of _Noreturn and of attribute("noreturn") *)
  if noret then
    warning loc Celeven_extension "_Noreturn functions are a C11 extension" ;
  if
    (noret || find_custom_attributes ["noreturn"; "__noreturn__"] attr <> [])
    && can_return
  then
    warning loc Invalid_noreturn
      "function '%s' declared 'noreturn' should not return" s ;
  ...
  let fn =
    ...
    ; fd_name= fun_id
    ; fd_attrib=
        (if noret then add_attributes [Attr ("noreturn", [])] attr else attr)
    ; fd_ret= ty_ret
    ; fd_params= params
    ; fd_vararg= vararg
    ; fd_locals= []
...
let rec elab_definition (local: bool) (env: Env.t) (def: Cabs.definition)
  ...
  | DECDEF (init_name_group, loc) ->
      let (dl, env1), sto, tydef =
        elab_init_name_group false loc env init_name_group
      in
      if tydef then
        let env2 = enter_typedefs loc env1 sto dl in
        ([], env2)
      else enter_decdefs local loc env1 sto dl
  (* pragma *)
  | PRAGMA (s, loc) ->
...
let stmt_labels stmt =
  ...
  let rec do_stmt = function
    ...
    | LABEL (lbl, s1, loc) ->
        ...
        do_stmt s1
    | _ -> ()
  and do_block b = List.iter do_stmt b in
  do_stmt stmt ; !lbls
...
let rec elab_stmt env ctx s =
  ...
  | CASE (a, s1, loc) ->
      ...
      | None ->
          error loc
            "expression of 'case' label is not an integer constant expression"
      | Some n -> () ) ;
      let s1, env = elab_stmt env ctx s1 in
  ...
  | BLOCK (b, loc) -> elab_block loc env ctx b
  (* 6.8.4 Conditional statements *)
  | If (a, s1, s2, loc) ->
      let a', env = elab_expr ctx.ctx_vararg loc env a in
      if not (is_scalar_type env a'.etyp) then
        error loc
          "controlling expression of 'if' does not have scalar type (%a \
           invalid)"
          (print_typ env) a'.etyp ;
      let s1', env = elab_stmt env ctx s1 in
      let s2', env =
        match s2 with None -> (sskip, env) | Some s2 -> elab_stmt env ctx s2
      in
  ...
  | WHILE (a, s1, loc) ->
      ...
      if not (is_scalar_type env a'.etyp) then
        error loc
          "controlling expression of 'while' does not have scalar type (%a \
           invalid)"
          (print_typ env) a'.etyp ;
  ...
  | FOR (fc, a2, a3, s1, loc) -> (
      let a1', env_decls, decls' =
        ...
        | Some (FC_DECL def) ->
            ...
            ( sskip
            , env'
            , Some (List.map (fun d -> {sdesc= Sdecl d; sloc= loc}) dcl) )
      in
      let a2', env_test =
        match a2 with
      ...
      if not (is_scalar_type env_test a2'.etyp) then
        error loc
          "controlling expression of 'for' does not have scalar type (%a \
           invalid)"
          (print_typ env) a2'.etyp ;
  ...
  | SWITCH (a, s1, loc) ->
      ...
      if not (is_integer_type env a'.etyp) then
        error loc
          "controlling expression of 'switch' does not have integer type (%a \
           invalid)"
          (print_typ env) a'.etyp ;
  ...
  | RETURN (a, loc) ->
      ...
      ( match (unroll env ctx.ctx_return_typ, a') with
      | TVoid _, None -> ()
      | TVoid _, Some _ ->
          error loc "'return' with a value in a function returning void"
      | _, None ->
          warning loc Return_type
            "'return' with no value, in a function returning non-void"
      | _, Some b ->
          if not (wrap2 valid_assignment loc env b ctx.ctx_return_typ) then
            if wrap2 valid_cast loc env b.etyp ctx.ctx_return_typ then
              if wrap2 int_pointer_conversion loc env b.etyp ctx.ctx_return_typ
              then
                warning loc Int_conversion
                  "incompatible integer-pointer conversion: returning %a from \
                   a function with result type %a"
                  (print_typ env) b.etyp (print_typ env) ctx.ctx_return_typ
              else
                warning loc Unnamed
                  "incompatible conversion returning %a from a function with \
                   result type %a"
                  (print_typ env) b.etyp (print_typ env) ctx.ctx_return_typ
            else
              error loc
  ...
  | NOP loc -> ({sdesc= Sskip; sloc= elab_loc loc}, env)
  (* Traditional extensions *)
  | ASM (cv_specs, wide, chars, outputs, inputs, flags, loc) ->
      let a = elab_cvspecs env cv_specs in
      let s = elab_simple_string loc wide chars in
      let outputs, env =
        mmap (elab_asm_operand ctx.ctx_vararg loc) env outputs
      in
      let inputs, env =
        mmap (elab_asm_operand ctx.ctx_vararg loc) env inputs
      in
      let flags = List.map (fun (w, c) -> elab_simple_string loc w c) flags in
      ({sdesc= Sasm (a, s, outputs, inputs, flags); sloc= elab_loc loc}, env)
  (* Unsupported *)
  | DEFINITION def ->
...
and elab_block_body env ctx sl =
  ...
  | DEFINITION def :: sl1 ->
      ...
      let dcl =
        List.map
          (fun ((sto, id, ty, _) as d) -> {sdesc= Sdecl d; sloc= loc})
          dcl
      in
