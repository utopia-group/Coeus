...
module Make =
 ...
 struct
  ...
  let is_goto_head_symbs _ =
    forallb (fun s ->
      forallb (fun nt ->
        match A.goto_table s nt with
        | Some s0 ->
          is_prefix (A.past_symb_of_non_init_state s0) (head_symbs_of_state s)
        | None -> true)
    ...
    (&&)
      ((&&)
        ((&&) ((&&) (is_shift_head_symbs ()) (is_goto_head_symbs ()))
          (is_shift_past_state ())) (is_goto_past_state ())) (is_reduce_ok ())
 end
