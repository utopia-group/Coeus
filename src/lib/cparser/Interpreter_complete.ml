...
module Make =
 ...
 struct
  ...
  | Cons_ptl_ptz of A.Gram.symbol * A.GramDefs.token list
     ...
     * A.GramDefs.token list * tuple * A.GramDefs.parse_tree_list * ptl_zipper
  and ptl_zipper =
  ...
  let pt_zipper_rect _ _ _ f f0 _ _ _ = function
  | Top_ptz -> f
  ...
  let pt_zipper_rec _ _ _ f f0 _ _ _ = function
  | Top_ptz -> f
  | Cons_ptl_ptz (x, x0, x1, x2, x3, x4, x5, x6) -> f0 x x0 x1 x2 x3 x4 x5 x6
  ...
  let rec ptl_zipper_rect init full_word full_sem f f0 _ _ _ = function
  ...
  let rec ptl_zipper_rec init full_word full_sem f f0 _ _ _ = function
  | Non_terminal_pt_ptlz (p0, word, semantic_values, p1) ->
    f p0 word semantic_values p1
  | Cons_ptl_ptlz (head_symbolt, wordt, semantic_valuet, p0, head_symbolsq,
                   wordq, semantic_valuesq, p1) ->
  ...
  let ptlz_cost _ _ _ =
    let rec ptlz_cost0 _ _ _ = function
    ...
    | Cons_ptl_ptlz (head_symbolt, wordt, semantic_valuet, _, head_symbolsq,
  ...
  let pt_dot_rect _ _ _ f f0 = function
  | Reduce_ptd x -> f x
  ...
  let pt_dot_rec _ _ _ f f0 = function
  | Reduce_ptd x -> f x
  | Shift_ptd (x, x0, x1, x2, x3, x4, x5) -> f0 x x0 x1 x2 x3 x4 x5
  ...
  | Shift_ptd (term, sem, symbolsq, wordq, semsq, ptl, ptlz) ->
    add (add (S O) (A.GramDefs.ptl_size symbolsq wordq semsq ptl))
  ...
  let ptlz_buffer _ _ buffer_end _ =
    let rec ptlz_buffer0 _ _ _ = function
    | Non_terminal_pt_ptlz (p, word, semantic_values, ptz) ->
    ...
    | Cons_ptl_ptlz (head_symbolt, wordt, semantic_valuet, _, head_symbolsq,
      ...
      Obj.magic ptlz_buffer0 (head_symbolt :: head_symbolsq)
        (app wordt wordq) (semantic_valuet, semantic_valuesq) ptlz'
    and ptz_buffer0 _ _ _ = function
    | Top_ptz -> buffer_end
    | Cons_ptl_ptz (head_symbolt, wordt, semantic_valuet, head_symbolsq,
                    wordq, semantic_valuesq, _, ptlz') ->
      Inter.app_str wordq
        (Obj.magic ptlz_buffer0 (head_symbolt :: head_symbolsq)
          (app wordt wordq) (semantic_valuet, semantic_valuesq) ptlz')
    in ptlz_buffer0
  ...
  let ptz_buffer _ _ buffer_end _ =
    let rec ptlz_buffer0 _ _ _ = function
    | Non_terminal_pt_ptlz (p, word, semantic_values, ptz) ->
      ptz_buffer0 (A.Gram.NT (A.Gram.prod_lhs p)) word
        (uncurry (map (Obj.magic __) (rev (A.Gram.prod_rhs_rev p)))
  ...
  | Shift_ptd (term, sem, symbolsq, wordq, semsq, _, ptlz) ->
  ...
  let rec ptlz_prod init full_word full_sem _ _ _ = function
  ...
  let rec ptlz_past init full_word full_sem _ _ _ = function
  | Non_terminal_pt_ptlz (_, _, _, _) -> []
  | Cons_ptl_ptlz (s, wordt, semantic_valuet, _, head_symbolsq, wordq,
                   semantic_valuesq, ptlz') ->
  ...
  let rec build_pt_dot init full_word full_sem _ _ _ ptl ptlz =
    match ptl with
    | A.GramDefs.Nil_ptl -> Reduce_ptd ptlz
    | A.GramDefs.Cons_ptl (_, _, _, pt, head_symbolsq, wordq,
                           semantic_valuesq, ptl') ->
      (match pt with
  ...
  let rec pop_ptlz init full_word full_sem _ _ _ ptl = function
  ...
  | Cons_ptl_ptlz (head_symbolt, wordt, semantic_valuet, pt, head_symbolsq,
    ...
    Obj.magic pop_ptlz init full_word full_sem
      ...
      semantic_valuet, pt, head_symbolsq, wordq, semantic_valuesq, ptl)) ptlz'
  ...
  let init_ptd init full_word full_sem full_pt =
    let x = Top_ptz in
    (match full_pt with
     | A.GramDefs.Terminal_pt (_, _) -> Obj.magic ()
     | A.GramDefs.Non_terminal_pt (p, word, semantic_values, ptl) ->
       build_pt_dot init full_word full_sem (rev (A.Gram.prod_rhs_rev p))
         word semantic_values ptl (Non_terminal_pt_ptlz (p, word,
         semantic_values, x)))
