...
let set_label_regpair lbl pos pos' subst =
  StringMap.add
    (name_of_label ~modifier:"R" lbl pos)
    (sprintf "%%R%d" pos')
    (StringMap.add
       (name_of_label ~modifier:"Q" lbl pos)
       (sprintf "%%Q%d" pos') subst)

let set_label_mem lbl pos pos' subst =
  let asm_mem_argument arg = Printf.sprintf "0(%s)" arg in
  StringMap.add (name_of_label lbl pos)
    (asm_mem_argument (sprintf "%%%d" pos'))
    subst
      ...
      if valid && String.contains cstr 'r' then
        if is_reg_pair env e.etyp then
          transf_inputs loc env (e :: accu) (pos + 1) (pos' + 1)
            (set_label_regpair lbl pos pos' subst)
            inputs
        else
          transf_inputs loc env (e :: accu) (pos + 1) (pos' + 1)
            (set_label_reg lbl pos pos' subst)
            inputs
      else if valid && String.contains cstr 'm' then
        transf_inputs loc env (eaddrof e :: accu) (pos + 1) (pos' + 1)
          (set_label_mem lbl pos pos' subst)
          inputs
      else if valid && (String.contains cstr 'i' || String.contains cstr 'n')
      then
        let n =
          match Ceval.integer_expr env e with
          | Some n -> n
          | None ->
              error loc "asm argument of kind '%s' is not a constant" cstr ;
              0L
        in
        transf_inputs loc env accu (pos + 1) pos'
          (set_label_const lbl pos n subst)
          inputs
      else (
        error loc "unsupported feature: asm argument of kind '%s'" cstr ;
        transf_inputs loc env (e :: accu) (pos + 1) (pos' + 1)
          (set_label_reg lbl pos pos' subst)
          inputs )
...
let transf_outputs loc env = function
  ...
  | [(lbl, cstr, e)] ->
      ...
      if valid && String.contains cstr 'r' then
        ...
        else (Some e, [], set_label_reg lbl 0 0 StringMap.empty, 1, 1)
      else if valid && String.contains cstr 'm' then
        (None, [eaddrof e], set_label_mem lbl 0 0 StringMap.empty, 1, 1)
      else (
        error loc "unsupported feature: asm result of kind '%s'" cstr ;
  ...
  | outputs ->
      ...
      in
      bind_outputs 0 StringMap.empty outputs
...
let rename_placeholders loc template subst =
  let rename p =
    if p = "%%" then p
      ...
      try StringMap.find p subst with Not_found ->
...
let transf_asm loc env template outputs inputs clobbers =
  let outputs', inputs1, subst1, pos, pos' = transf_outputs loc env outputs in
  let inputs', subst = transf_inputs loc env inputs1 pos pos' subst1 inputs in
