...
type inline_status =
  | No_specifier
  (* No inline specifier and no noinline attribute *)
  | Noinline
  (* The atom is declared with the noinline attribute *)
  | Inline

(* The atom is declared inline *)

type atom_info =
  { a_storage: C.storage
  ; (* storage class *)
    a_alignment: int option
  ; (* alignment *)
    a_sections: Sections.section_name list
  ; (* in which section to put it *)
    (* 1 section for data, 3 sections (code/lit/jumptbl) for functions *)
    a_access: Sections.access_mode
  ; (* access mode, e.g. small data area *)
...
let atom_is_static a =
  try (Hashtbl.find decl_atom a).a_storage = C.Storage_static
  with Not_found -> false

let atom_is_extern a =
  try (Hashtbl.find decl_atom a).a_storage = C.Storage_extern
  with Not_found -> false

let atom_alignof a =
  try (Hashtbl.find decl_atom a).a_alignment with Not_found -> None

let atom_sections a =
  try (Hashtbl.find decl_atom a).a_sections with Not_found -> []

let atom_is_small_data a ofs =
  try (Hashtbl.find decl_atom a).a_access = Sections.Access_near
  with Not_found -> false

let atom_is_rel_data a ofs =
  try (Hashtbl.find decl_atom a).a_access = Sections.Access_far
  with Not_found -> false

let atom_inline a =
  try (Hashtbl.find decl_atom a).a_inline with Not_found -> No_specifier
...
let atom_location a =
  try (Hashtbl.find decl_atom a).a_loc with Not_found -> Cutil.no_loc
...
let error fmt = Cerrors.error !currentLocation fmt

let unsupported msg =
  Cerrors.error !currentLocation "unsupported feature: %s" msg

let warning t msg = Cerrors.warning !currentLocation t msg
...
let builtins_generic =
  ...
  ; Builtins.functions=
     ...
     * @ *)
      [ (* Integer arithmetic *)
        ("__builtin_bswap", (TInt (IUInt, []), [TInt (IUInt, [])], false))
      ; ("__builtin_bswap32", (TInt (IUInt, []), [TInt (IUInt, [])], false))
      ; ("__builtin_bswap16", (TInt (IUShort, []), [TInt (IUShort, [])], false))
      ...
      ; (* Annotations *)
        ( "__builtin_annot"
        , (TVoid [], [TPtr (TInt (IChar, [AConst]), [])], true) )
      ; ( "__builtin_annot_intval"
        , ( TInt (IInt, [])
          , [TPtr (TInt (IChar, [AConst]), []); TInt (IInt, [])]
          , false ) )
      ; (* Software memory barrier *)
        ("__builtin_membar", (TVoid [], [], false))
      ; (* Variable arguments *)
        (* va_start(ap,n)
      (preprocessing) --> __builtin_va_start(ap, arg)
      (elaboration)   --> __builtin_va_start(ap) *)
        ("__builtin_va_start", (TVoid [], [TPtr (TVoid [], [])], false))
      ; (* va_arg(ap, ty)
      (preprocessing) --> __builtin_va_arg(ap, ty)
      (parsing)       --> __builtin_va_arg(ap, sizeof(ty)) *)
        ( "__builtin_va_arg"
        , (TVoid [], [TPtr (TVoid [], []); TInt (IUInt, [])], false) )
      ; ( "__builtin_va_copy"
        , (TVoid [], [TPtr (TVoid [], []); TPtr (TVoid [], [])], false) )
      ; ("__builtin_va_end", (TVoid [], [TPtr (TVoid [], [])], false))
      ; ( "__compcert_va_int32"
        , (TInt (IUInt, []), [TPtr (TVoid [], [])], false) )
      ; ( "__compcert_va_int64"
        , (TInt (IULongLong, []), [TPtr (TVoid [], [])], false) )
      ; ( "__compcert_va_float64"
        , (TFloat (FDouble, []), [TPtr (TVoid [], [])], false) )
      ; ( "__compcert_va_composite"
        , (TPtr (TVoid [], []), [TPtr (TVoid [], []); TInt (IULong, [])], false)
        )
      ; (* Helper functions for int64 arithmetic *)
        ( "__compcert_i64_dtos"
        , (TInt (ILongLong, []), [TFloat (FDouble, [])], false) )
      ; ( "__compcert_i64_dtou"
        , (TInt (IULongLong, []), [TFloat (FDouble, [])], false) )
      ; ( "__compcert_i64_stod"
        , (TFloat (FDouble, []), [TInt (ILongLong, [])], false) )
      ; ( "__compcert_i64_utod"
        , (TFloat (FDouble, []), [TInt (IULongLong, [])], false) )
      ; ( "__compcert_i64_stof"
        , (TFloat (FFloat, []), [TInt (ILongLong, [])], false) )
      ; ( "__compcert_i64_utof"
        , (TFloat (FFloat, []), [TInt (IULongLong, [])], false) )
      ; ( "__compcert_i64_sdiv"
        , ( TInt (ILongLong, [])
      ...
      ; ( "__compcert_i64_umod"
        , ( TInt (IULongLong, [])
          ...
          , false ) )
      ; ( "__compcert_i64_shl"
        , (TInt (ILongLong, []), [TInt (ILongLong, []); TInt (IInt, [])], false)
        )
      ; ( "__compcert_i64_shr"
        , ( TInt (IULongLong, [])
...
let builtins =
  ...
  { typedefs= builtins_generic.typedefs
  ; functions= builtins_generic.Builtins.functions }
...
(* number of next global for string literals *)
...
let name_for_string_literal s =
  try Hashtbl.find stringTable s with Not_found ->
    incr stringNum ;
...
let global_for_string s id =
  let init = ref [] in
  let add_char c = init := AST.Init_int8 (Z.of_uint (Char.code c)) :: !init in
  add_char '\000' ;
  for i = String.length s - 1 downto 0 do add_char s.[i] done ;
  ( id
  , AST.Gvar
      { AST.gvar_info= typeStringLiteral s
      ; AST.gvar_init= !init
      ; AST.gvar_readonly= true
      ; AST.gvar_volatile= false } )

let name_for_wide_string_literal s =
  try Hashtbl.find wstringTable s with Not_found ->
    incr stringNum ;
    let name = Printf.sprintf "__stringlit_%d" !stringNum in
    let id = intern_string name in
    Hashtbl.add decl_atom id
...
let typeWideStringLiteral s =
  let sz =
    ...
    | _ -> assert false
  in
  let sg = if Machine.(!config.wchar_signed) then Signed else Unsigned in
...
let global_for_wide_string s id =
  ...
  let init_of_char =
    ...
    | _ -> assert false
  in
  let add_char c = init := init_of_char (Z.of_uint64 c) :: !init in
  List.iter add_char s ;
  add_char 0L ;
  let open AST in
  ( id
  , Gvar
      { gvar_info= typeWideStringLiteral s
...
let globals_for_strings globs =
  ...
  in
  let globs2 =
    Hashtbl.fold (fun s id l -> global_for_string s id :: l) stringTable globs1
  in
  globs2
...
let make_builtin_memcpy args =
  ...
  | Econs (dst, Econs (src, Econs (sz, Econs (al, Enil)))) ->
      let sz1 =
        ...
        | None ->
            error
              "size argument of '__builtin_memcpy_aligned' must be a constant" ;
            Z.zero
      in
      let al1 =
        ...
        | None ->
            error
              "alignment argument of '__builtin_memcpy_aligned' must be a \
               constant" ;
            Z.one
      in
      if not (Z.is_power2 al1) then
        error
          "alignment argument of '__builtin_memcpy_aligned' must be a power \
           of 2" ;
      if not (Z.eq (Z.modulo sz1 al1) Z.zero) then
        error
          "alignment argument of '__builtin_memcpy_aligned' must be a divisor \
           of the size" ;
      (* Issue #28: must decay array types to pointer types *)
      Ebuiltin
        ( AST.EF_memcpy (sz1, al1)
        , Tcons (typeconv (typeof dst), Tcons (typeconv (typeof src), Tnil))
        , Econs (dst, Econs (src, Enil))
        , Tvoid )
  | _ -> assert false
...
(* if not CBuiltins.va_list_scalar then e
   * else
   *   match e with
   *   | Evalof (e', _) -> Eaddrof (e', Tpointer (typeof e, noattr))
   *   | _ ->
   *       error "bad use of a va_list object" ;
   *       e *)

let make_builtin_va_arg_by_val helper ty ty_ret arg =
  let ty_fun =
    Tfunction (Tcons (Tpointer (Tvoid, noattr), Tnil), ty_ret, AST.cc_default)
  in
  Ecast
    ( Ecall
        ( Evalof (Evar (intern_string helper, ty_fun), ty_fun)
...
let make_builtin_va_arg_by_ref helper ty arg =
  let ty_fun =
    Tfunction
      ( Tcons (Tpointer (Tvoid, noattr), Tcons (Ctyping.size_t, Tnil))
      , Tpointer (Tvoid, noattr)
      , AST.cc_default )
  in
  let ty_ptr = Tpointer (ty, noattr) in
  let call =
    Ecall
      ( Evalof (Evar (intern_string helper, ty_fun), ty_fun)
...
let make_builtin_va_arg env ty e =
  match ty with
  | Ctypes.Tint _ ->
      make_builtin_va_arg_by_val "__compcert_va_int32" ty
        (Tint (I32, Unsigned, noattr)) e
  | Tpointer _ when Archi.ptr64 = false ->
      make_builtin_va_arg_by_val "__compcert_va_int32" ty
        (Tint (I32, Unsigned, noattr)) e
  | Tpointer _ when Archi.ptr64 = true ->
      make_builtin_va_arg_by_val "__compcert_va_int64" ty
        (Tlong (Unsigned, noattr)) e
  | Tlong _ ->
      make_builtin_va_arg_by_val "__compcert_va_int64" ty
        (Tlong (Unsigned, noattr)) e
  | Tfloat _ ->
      make_builtin_va_arg_by_val "__compcert_va_float64" ty
        (Tfloat (F64, noattr)) e
  | Tstruct _ | Tunion _ ->
      make_builtin_va_arg_by_ref "__compcert_va_composite" ty e
  | _ ->
...
let convertCallconv va unproto attr =
  let sr =
    Cutil.find_custom_attributes ["structreturn"; "__structreturn"] attr
  in
...
let convertIkind k a : coq_type =
  ...
  | C.IChar ->
      Tint (I8, (if Machine.(!config.char_signed) then Signed else Unsigned), a)
  ...
  | C.ILong ->
      if Machine.(!config.sizeof_long) = 8 then Tlong (Signed, a)
      else Tint (I32, Signed, a)
  | C.IULong ->
      if Machine.(!config.sizeof_long) = 8 then Tlong (Unsigned, a)
      else Tint (I32, Unsigned, a)
...
let checkFunctionType env tres targs =
  if not !Clflags.option_fstruct_passing then (
    if Cutil.is_composite_type env tres then
      unsupported
        "function returning a struct or union (consider adding option \
         -fstruct-passing)" ;
    match targs with
    | None -> ()
    | Some l ->
        List.iter
          (fun (id, ty) ->
            if Cutil.is_composite_type env ty then
              unsupported
                "function parameter of struct or union type (consider adding \
                 option -fstruct-passing)" )
          l )

let rec convertTyp env t =
  match t with
  | C.TVoid a -> Tvoid
  | C.TInt (ik, a) -> convertIkind ik (convertAttr a)
  | C.TFloat (fk, a) -> convertFkind fk (convertAttr a)
  | C.TPtr (ty, a) -> Tpointer (convertTyp env ty, convertAttr a)
  | C.TArray (ty, None, a) ->
  ...
  | C.TFun (tres, targs, va, a) ->
      ...
      Tfunction
        ( (match targs with None -> Tnil | Some tl -> convertParams env tl)
        , convertTyp env tres
        , convertCallconv va (targs = None) a )
  | C.TNamed _ -> convertTyp env (Cutil.unroll env t)
  | C.TStruct (id, a) -> Ctypes.Tstruct (intern_string id.name, convertAttr a)
  | C.TUnion (id, a) -> Tunion (intern_string id.name, convertAttr a)
  | C.TEnum (id, a) -> convertIkind Cutil.enum_ikind (convertAttr a)
...
let convertField env f =
  if f.fld_bitfield <> None then
    unsupported
      "bit field in struct or union (consider adding option -fbitfields)" ;
  (intern_string f.fld_name, convertTyp env f.fld_typ)

let convertCompositedef env su id attr members =
  Composite
    ( intern_string id.name
    , (match su with C.Struct -> Ctypes.Struct | C.Union -> Ctypes.Union)
...
let checkFloatOverflow f typ =
  ...
  | Fappli_IEEE.B754_zero _ ->
      warning Cerrors.Literal_range
        "magnitude of floating-point constant too small for type '%s'" typ
  | Fappli_IEEE.B754_infinity _ ->
      warning Cerrors.Literal_range
        "magnitude of floating-point constant too large for type '%s'" typ
...
let convertFloat f kind =
  ...
  | Z.Z0 -> (
    match kind with
    | FFloat -> Ctyping.econst_single (Float.to_single Float.zero)
    | FDouble | FLongDouble -> Ctyping.econst_float Float.zero )
  | Z.Zpos mant -> (
      ...
      let shift_exp =
        (if f.C.hex then 4 else 1) * String.length f.C.fracPart
      in
...
let ewrap = function
  | Errors.OK e -> e
  | Errors.Error msg ->
      error "retyping error: %s" (string_of_errmsg msg) ;
      ezero
...
let rec convertExpr env e =
  ...
  | C.EConst (C.CInt (i, k, _)) ->
      ...
      if Cutil.sizeof_ikind k = 8 then
        Ctyping.econst_long (coqint_of_camlint64 i) sg
  ...
  | C.EConst (C.CEnum (id, i)) -> Ctyping.econst_int (convertInt i) Signed
  | C.ESizeof ty1 -> Ctyping.esizeof (convertTyp env ty1)
  | C.EAlignof ty1 -> Ctyping.ealignof (convertTyp env ty1)
  | C.EUnop (C.Ominus, e1) ->
      ewrap (Ctyping.eunop Cop.Oneg (convertExpr env e1))
  | C.EUnop (C.Oplus, e1) -> convertExpr env e1
  ...
  | C.EUnop (C.Oaddrof, e1) -> ewrap (Ctyping.eaddrof (convertLvalue env e1))
  | C.EUnop (C.Opreincr, e1) -> ewrap (Ctyping.epreincr (convertLvalue env e1))
  | C.EUnop (C.Opredecr, e1) -> ewrap (Ctyping.epredecr (convertLvalue env e1))
  ...
  | C.EBinop
      ( ( ( C.Oadd | C.Osub | C.Omul | C.Odiv | C.Omod | C.Oand | C.Oor
          | C.Oxor | C.Oshl | C.Oshr | C.Oeq | C.One | C.Olt | C.Ogt | C.Ole
          | C.Oge ) as op )
      , e1
      , e2
      , tyres ) ->
  ...
  | C.EBinop (C.Oassign, e1, e2, _) ->
      ...
      if
        Cutil.is_composite_type env e1.etyp
        && List.mem AVolatile (Cutil.attributes_of_type env e1.etyp)
      then
        warning Cerrors.Unnamed
          "assignment to an lvalue of volatile composite type, the 'volatile' \
           qualifier is ignored" ;
      ewrap (Ctyping.eassign e1' e2')
  | C.EBinop
      ( ( ( C.Oadd_assign | C.Osub_assign | C.Omul_assign | C.Odiv_assign
  ...
  | C.EConditional (e1, e2, e3) ->
      ewrap
        (Ctyping.econdition (convertExpr env e1) (convertExpr env e2)
           (convertExpr env e3))
  | C.ECast (ty1, e1) ->
      ewrap (Ctyping.ecast (convertTyp env ty1) (convertExpr env e1))
  | C.ECompound (ty1, ie) ->
      unsupported "compound literals" ;
      ezero
  | C.ECall ({edesc= C.EVar {name= "__builtin_debug"}}, args) ->
      ...
      let kind, args1 =
        ...
        | _ :: args ->
            error "argument 1 of '__builtin_debug' must be a non-zero constant" ;
            (1L, args)
        | [] -> assert false
        (* catched earlier *)
      in
      let text, args2 =
        ...
        | _ :: args2 ->
            error
              "argument 2 of '__builtin_debug' must be either a string \
      ...
      Ebuiltin
        ( AST.EF_debug
            (P.of_int64 kind, intern_string text, typlist_of_typelist targs2)
        , targs2
        , convertExprList env args2
  ...
  | C.ECall ({edesc= C.EVar {name= "__builtin_annot"}}, args) -> (
    ...
    | {edesc= C.EConst (CStr txt)} :: args1 ->
        ...
        Ebuiltin
          ( AST.EF_annot
              ( P.of_int 1
              , coqstring_of_camlstring txt
              , typlist_of_typelist targs1 )
          , targs1
          , convertExprList env args1
          , convertTyp env e.etyp )
    | _ ->
  ...
  | C.ECall ({edesc= C.EVar {name= "__builtin_annot_intval"}}, args) -> (
    ...
    | [{edesc= C.EConst (CStr txt)}; arg] ->
        let targ =
          convertTyp env (Cutil.default_argument_conversion env arg.etyp)
        in
        Ebuiltin
          ( AST.EF_annot_val
              (P.of_int 1, coqstring_of_camlstring txt, typ_of_type targ)
          , Tcons (targ, Tnil)
          , convertExprList env [arg]
          , convertTyp env e.etyp )
    | _ ->
  ...
  | C.ECall ({edesc= C.EVar {name= "__builtin_va_copy"}}, [arg1; arg2]) ->
      ...
      Ebuiltin
        ( AST.EF_memcpy (Z.of_uint (3 * 8), Z.of_uint 4)
        , Tcons
            (Tpointer (Tvoid, noattr), Tcons (Tpointer (Tvoid, noattr), Tnil))
    ...
    when !Clflags.option_interp ->
      ...
      let sg =
        signature_of_type targs tres
          {AST.cc_vararg= true; cc_unproto= false; cc_structret= false}
      in
      Ebuiltin
        ( AST.EF_external (coqstring_of_camlstring "printf", sg)
        , targs
        , convertExprList env args
        , tres )
  | C.ECall (fn, args) ->
      ( match projFunType env fn.etyp with
      | None -> error "wrong type for function part of a call"
      | Some (tres, targs, va) ->
          checkFunctionType env tres targs ;
          if targs = None && not !Clflags.option_funprototyped then
            unsupported
              "call to unprototyped function (consider adding option \
               [-funprototyped])" ;
          if va && not !Clflags.option_fvararg_calls then
            unsupported
              "call to variable-argument function (consider adding option \
               [-fvararg-calls])" ) ;
...
and convertLvalue env e =
  ...
  | C.EUnop (C.Oderef, e1) -> ewrap (Ctyping.ederef (convertExpr env e1))
...
let convertAsm loc env txt outputs inputs clobber =
  ...
  in
  let clobber' =
    List.map (fun s -> coqstring_uppercase_ascii_of_camlstring s) clobber
  in
  let ty_res = match output' with None -> TVoid [] | Some e -> e.etyp in
...
type switchlabel = Case of C.exp | Default

type switchbody = Label of switchlabel | Stmt of C.stmt

let rec flattenSwitch = function
  | {sdesc= C.Sseq (s1, s2)} -> flattenSwitch s1 @ flattenSwitch s2
  | {sdesc= C.Slabeled (C.Scase e, s1)} -> Label (Case e) :: flattenSwitch s1
  | {sdesc= C.Slabeled (C.Sdefault, s1)} -> Label Default :: flattenSwitch s1
...
let rec contains_case s =
  match s.sdesc with
  | C.Sskip | C.Sdo _ | C.Sbreak | C.Scontinue | C.Sswitch _
  (* Stop at a switch *)  | C.Sgoto _ | C.Sreturn _ | C.Sdecl _ | C.Sasm _ ->
      ()
  | C.Sseq (s1, s2) | C.Sif (_, s1, s2) -> contains_case s1 ; contains_case s2
  | C.Swhile (_, s1) | C.Sdowhile (s1, _) -> contains_case s1
  | C.Sfor (s1, e, s2, s3) ->
      contains_case s1 ; contains_case s2 ; contains_case s3
...
let rec convertStmt env s =
  updateLoc s.sloc ;
  match s.sdesc with
  | C.Sskip -> Csyntax.Sskip
  | C.Sdo e -> swrap (Ctyping.sdo (convertExpr env e))
  | C.Sseq (s1, s2) ->
  ...
  | C.Sfor (s1, e, s2, s3) ->
      let te = convertExpr env e in
      swrap
        (Ctyping.sfor (convertStmt env s1) te (convertStmt env s2)
           (convertStmt env s3))
  | C.Sbreak -> Csyntax.Sbreak
  | C.Scontinue -> Csyntax.Scontinue
  | C.Sswitch (e, s1) ->
      ...
      let te = convertExpr env e in
      swrap
        (Ctyping.sswitch te (convertSwitch env (is_int64 env e.etyp) cases))
  ...
  | C.Slabeled (C.Sdefault, _) ->
      unsupported "'default' statement not in 'switch' statement" ;
      Csyntax.Sskip
  | C.Sgoto lbl -> Csyntax.Sgoto (intern_string lbl)
  | C.Sreturn None -> Csyntax.Sreturn None
  | C.Sreturn (Some e) -> Csyntax.Sreturn (Some (convertExpr env e))
  | C.Sblock _ ->
      unsupported "nested blocks" ;
      Csyntax.Sskip
  | C.Sdecl _ ->
  ...
  | C.Sasm (attrs, txt, outputs, inputs, clobber) ->
      if not !Clflags.option_finline_asm then
        unsupported
          "inline 'asm' statement (consider adding option [-finline-asm])" ;
  ...
  | (lbl, s) :: rem ->
      ...
      let lbl' =
        ...
        | Case e ->
          ...
          | None ->
              unsupported "expression is not an integer constant expression" ;
              None
          | Some v ->
              Some
                ( if is_64 then Z.of_uint64 v
...
let convertFundef loc env fd =
  ...
  if fd.fd_vararg && not !Clflags.option_fvararg_calls then
    unsupported
      "variable-argument function (consider adding option [-fvararg-calls])" ;
  let ret = convertTyp env fd.fd_ret in
  let params =
    List.map
      (fun (id, ty) ->
  ...
  let vars =
    List.map
      (fun (sto, id, ty, init) ->
        ...
        if init <> None then unsupported "initialized local variable" ;
        let id' = intern_string id.name in
        (id', convertTyp env ty) )
      fd.fd_locals
  in
  ...
  let noinline =
    Cutil.find_custom_attributes ["noinline"; "__noinline__"] fd.fd_attrib
    <> []
  in
  let inline =
    if noinline || fd.fd_vararg then (* PR#15 *)
      Noinline
    else if fd.fd_inline then Inline
    else No_specifier
  in
  Hashtbl.add decl_atom id'
...
let convertFundecl env (sto, id, ty, optinit) =
  ...
  let ef =
    if id.name = "malloc" then AST.EF_malloc
    else if id.name = "free" then AST.EF_free
    else if Str.string_match re_runtime id.name 0 then AST.EF_runtime (id'', sg)
    else if
      Str.string_match re_builtin id.name 0
      && List.mem_assoc id.name builtins.Builtins.functions
    then AST.EF_builtin (id'', sg)
    else AST.EF_external (id'', sg)
  in
...
let rec convertInit env init =
  match init with
  | C.Init_single e -> Initializers.Init_single (convertExpr env e)
  | C.Init_array il ->
      Initializers.Init_array
        (convertInitList env (List.rev il) Initializers.Init_nil)
  | C.Init_struct (_, flds) ->
      Initializers.Init_struct
        (convertInitList env (List.rev_map snd flds) Initializers.Init_nil)
...
and convertInitList env il accu =
  ...
  | [] -> accu
  | i :: il' ->
      convertInitList env il'
        (Initializers.Init_cons (convertInit env i, accu))

let convertInitializer env ty i =
  match
    Initializers.transl_init !comp_env (convertTyp env ty) (convertInit env i)
  with
  | Errors.OK init -> init
  | Errors.Error msg ->
...
let convertGlobvar loc env (sto, id, ty, optinit) =
  ...
  let init' =
    match optinit with
    | None -> if sto = C.Storage_extern then [] else [AST.Init_space sz]
    | Some i -> convertInitializer env ty i
  in
  let section, access = Sections.for_variable env id' ty (optinit <> None) in
  if Z.gt sz (Z.of_uint64 0xFFFF_FFFFL) then
    error "'%s' is too big (%s bytes)" id.name (Z.to_string sz) ;
  ...
  | g :: gl' ->
      ...
      | C.Gdecl ((sto, id, ty, optinit) as d) -> (
        ...
        | TFun (tres, targs, va, a) ->
            if targs = None then
              warning Cerrors.Unnamed
                "'%s' is declared without a function prototype" id.name ;
            convertGlobdecls env (convertFundecl env d :: res) gl'
        | _ -> convertGlobdecls env (convertGlobvar g.gloc env d :: res) gl' )
      | C.Gfundef fd ->
  ...
  | g :: gl' ->
      ...
      | C.Gcompositedef (su, id, a, m) ->
          convertCompositedefs env
            (convertCompositedef env su id a m :: res)
            gl'
      | _ -> convertCompositedefs env res gl'
...
let rec translEnv env = function
  ...
  | g :: gl ->
      let env' =
        ...
        | C.Gtypedef (id, ty) -> Env.add_typedef env id ty
...
let cleanupGlobals p =
  ...
  let strong = ref IdentSet.empty
  (* def functions or variables with inits *)
  and weak = ref IdentSet.empty
  (* variables without inits *)
  and extern = ref IdentSet.empty in
  (* extern decls *)
  let classify_def g =
    updateLoc g.gloc ;
    match g.gdesc with
    ...
    | C.Gdecl (sto, id, ty, None) -> weak := IdentSet.add id !weak
    | _ -> ()
  in
    ...
    | g :: gl ->
        ...
        | C.Gdecl (sto, id, ty, init) ->
            let better_def_exists =
              ...
              else if init = None then IdentSet.mem id !strong
              else false
            in
            if IdentSet.mem id defs || better_def_exists then
              clean defs accu gl
            else clean (IdentSet.add id defs) (g :: accu) gl
        | C.Gfundef fd -> clean (IdentSet.add fd.fd_name defs) (g :: accu) gl
        | _ -> clean defs (g :: accu) gl
  in
  clean IdentSet.empty [] (List.rev p)
...
let convertProgram p =
  ...
  try
    ...
    | Errors.Error msg ->
        error "incorrect struct or union definition: %s" (string_of_errmsg msg) ;
        None
    | Errors.OK ce ->
        comp_env := ce ;
