...
module InternalStatus = struct
  ...
  let to_external_status = function
...
module Discharger = struct
  ...
  let discharge (z3env: Z3Env.t) pre_conds opt_post_conds =
    ...
    let z3_posts =
      ...
      | None -> [Z3.Boolean.mk_false z3env.ctx]
      | Some post_conds -> List.map post_conds ~f:(Z3Encoder.encode_expr z3env)
    in
    let z3_query =
      ...
      | Some post_conds ->
        ...
        | Some model ->
            ...
            List.iteri z3_posts ~f:(fun idx z3_post ->
                ...
                | Some e when Z3.Boolean.is_true e -> Hash_set.add keep_set idx
                | _ -> (* Be conservative and filter out this expression *)
                       () ) ;
...
let run_houdini z3env ast =
        ...
        List.fold rules ~init:InternalStatus.Verified ~f:(fun acc rule ->
            ...
            | _ ->
                ...
                let rule_status =
                  ...
                  | Status.Verified -> InternalStatus.Verified
                  | Status.Rejected -> InternalStatus.Rejected
                  | Status.Pruned keep_set ->
  ...
  in
  let rec iterate_until_fixpoint acc elem =
    match acc with
    | InternalStatus.Rejected -> acc
    | _ ->
        let open Graph.WeakTopological in
        let open InternalStatus in
        let elem_status =
          match elem with
          | Vertex v -> process_pred v
          | Component (v, elem') ->
              let rec iterate () =
                let vstatus = process_pred v in
                match fold_left iterate_until_fixpoint vstatus elem' with
                | (Rejected | Verified) as s -> s
                | Undeterminted -> iterate ()
              in
              iterate ()
        in
        merge acc elem_status
  in
  let internal_status =
    Graph.WeakTopological.fold_left iterate_until_fixpoint
      InternalStatus.Verified weak_topo
  in
  Logs.debug (fun m ->
      SolutionTable.iteri solution_table ~f:(fun ~key ~data ->
          let pp_and fmt () = Fmt.string fmt " && " in
          m " Final Houdini solution for %a: %a" Identifier.pp key
            (Fmt.list ~sep:pp_and Expr.pp)
            data ) ) ;
  InternalStatus.to_external_status internal_status
