(** Directly copied from Core_extended.Cache. *)
...
struct
  ...
  let to_alist cache = Store.to_alist cache.store

  let length cache = Store.length cache.store
...
let keep_one ?(destruct= ignore) f =
  ...
  fun x ->
    match !v with
    | Some (x', y) when x' = x -> Memoized.return y
    | _ ->
...
let memoize ?destruct ?(expire= `Keep_all) f =
  match expire with
  | `Lru size -> snd (Lru.memoize ?destruct f size)
  | `Keep_all -> snd (Keep_all.memoize ?destruct f)
